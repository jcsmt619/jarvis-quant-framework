...
from core.research_next_cycle_records_review_gate_33 import (
    ...
    build_default_research_next_cycle_records_review_gate_33_input,
    build_research_next_cycle_records_review_gate_33_payload,
    render_research_next_cycle_records_review_gate_33_markdown,
    write_research_next_cycle_records_review_gate_33,
)
...
def _root() -> Path:
    return Path("reports/research_next_cycle_records_review_gate_33_tests") / uuid.uuid4().hex
...
def _gate_input(root: Path) -> ResearchNextCycleRecordsReviewGate33Input:
    return ResearchNextCycleRecordsReviewGate33Input(
        records_review_gate_33_id="33A-RESEARCH-NEXT-CYCLE-RECORDS-REVIEW-GATE-33-2026-07-07",
        records_review_gate_33_date="2026-07-07",
...
def _write_ready_inputs(root: Path) -> ResearchNextCycleRecordsReviewGate33Input:
    ...
    payloads = {
        ...
        gate_input.frozen_start_evidence_packet_path: {
            **_artifact_payload("30B", "Next Cycle Frozen Start Evidence Packet", "frozen_start_evidence_packet_date"),
            ...
            "inert_command_hints": [
                {
                    "hint_id": "30B-HINT-33A",
                    "label": HUMAN_REVIEW_REQUIRED,
                    "summary": "Review records gate.",
                    "command_hint": "python scripts/run_research_next_cycle_records_review_gate_33.py",
                    "would_run": False,
                    "executed": False,
                }
            ],
    ...
    _write_json(
        ...
        [
            {
                "phase": "33A",
                "title": "Next Cycle Records Review Gate 33",
                "spec": "Build deterministic Records Review packet records.",
            },
        ],
    )
    return gate_input
...
def test_33A_builds_deterministic_records_review_gate_33() -> None:
    root = _root()
    try:
        gate_input = _write_ready_inputs(root)

        first = build_research_next_cycle_records_review_gate_33_payload(gate_input)
        second = build_research_next_cycle_records_review_gate_33_payload(gate_input)

        assert first == second
        assert first["phase"] == "33A"
        assert first["workflow"] == "Next Cycle Records Review Gate 33"
        ...
        assert first["summary"]["evidence_reference_count"] == 16
        assert first["queue_next_phase"]["phase"] == "33A"
...
def test_33A_writes_json_and_markdown_gate_33() -> None:
    root = _root()
    out_dir = root / "out"
    try:
        json_path, markdown_path = write_research_next_cycle_records_review_gate_33(
        ...
        assert json_path.name == "research_next_cycle_records_review_gate_33.json"
        assert markdown_path.name == "research_next_cycle_records_review_gate_33.md"
        assert payload["records_review_gate_33_id"] == "33A-RESEARCH-NEXT-CYCLE-RECORDS-REVIEW-GATE-33-2026-07-07"
        assert "33A Next Cycle Records Review Gate 33" in markdown
        assert "Final Records Review Gate 33 Summary" in markdown
...
def test_33A_classifies_blocked_for_missing_source_and_failed_safety_scanner() -> None:
    root = _root()
    try:
        gate_input = _write_ready_inputs(root)
        ...
        markdown = render_research_next_cycle_records_review_gate_33_markdown(payload)

        assert payload["records_review_gate_33_state"] == BLOCKED_BY_SAFETY_GATE
...
def test_33A_rejects_secret_paths_unsafe_labels_and_execution_flags() -> None:
    root = _root()
    try:
        with pytest.raises(ValueError, match="secret files"):
            ResearchNextCycleRecordsReviewGate33Input(
                records_review_gate_33_id="33A-UNSAFE",
                records_review_gate_33_date="2026-07-07",
                generated_at_utc=FIXED_NOW.isoformat(),
        ...
        with pytest.raises(ValueError, match="unsafe|disallowed"):
            build_research_next_cycle_records_review_gate_33_payload(gate_input)
...
def test_33A_default_input_uses_phase_gate_id() -> None:
    gate_input = build_default_research_next_cycle_records_review_gate_33_input(now=FIXED_NOW)

    assert gate_input.records_review_gate_33_id == "33A-RESEARCH-NEXT-CYCLE-RECORDS-REVIEW-GATE-33-2026-07-07"
    assert gate_input.records_review_gate_33_date == "2026-07-07"


def test_33A_cli_writes_next_cycle_records_review_gate_33() -> None:
    root = _root()
    out_dir = root / "out"
    try:
        gate_input = _write_ready_inputs(root)
        completed = subprocess.run(
            [
                sys.executable,
                "scripts/run_research_next_cycle_records_review_gate_33.py",
                "--out-dir",
                str(out_dir),
                "--records-review-gate-33-date",
                "2026-07-07",
        ...
        assert "JARVIS 33A NEXT CYCLE RECORDS REVIEW GATE 33: COMPLETE" in completed.stdout
        assert "Records Review Gate 33 is read-only and records-only" in completed.stdout
        ...
        assert "Records Review Gate 33 records do not grant execution permission" in completed.stdout
        assert "LIVE TRADING: DISABLED" in completed.stdout
        assert (out_dir / "research_next_cycle_records_review_gate_33.json").exists()
        assert (out_dir / "research_next_cycle_records_review_gate_33.md").exists()
    finally:
        shutil.rmtree(root, ignore_errors=True)
