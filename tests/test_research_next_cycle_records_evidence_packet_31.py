...
from core.research_next_cycle_records_evidence_packet_31 import (
    ...
    build_research_next_cycle_records_evidence_packet_31_payload,
    render_research_next_cycle_records_evidence_packet_31_markdown,
...
def _root() -> Path:
    return Path("reports/research_next_cycle_records_evidence_packet_31_tests") / uuid.uuid4().hex
...
def _gate_input(root: Path) -> ResearchNextCycleRecordsEvidencePacket31Input:
    return ResearchNextCycleRecordsEvidencePacket31Input(
        records_evidence_packet_31_id="31B-RESEARCH-NEXT-CYCLE-RECORDS-EVIDENCE-PACKET-31-2026-07-07",
        records_evidence_packet_31_date="2026-07-07",
...
def _write_ready_inputs(root: Path) -> ResearchNextCycleRecordsEvidencePacket31Input:
    ...
    payloads = {
        ...
        gate_input.frozen_start_evidence_packet_path: {
            **_artifact_payload("30B", "Next Cycle Frozen Start Evidence Packet", "frozen_start_evidence_packet_date"),
            ...
            "inert_command_hints": [
                {
                    "hint_id": "30B-HINT-31B",
                    "label": HUMAN_REVIEW_REQUIRED,
                    "summary": "Review records gate.",
                    "command_hint": "python scripts/run_research_next_cycle_records_evidence_packet_31.py",
                    "would_run": False,
                    "executed": False,
                }
            ],
    ...
    _write_json(
        ...
        [
            {
                "phase": "31B",
                "title": "Next Cycle Records Evidence Packet 31",
...
def test_31B_builds_deterministic_records_evidence_packet_31() -> None:
    root = _root()
    try:
        gate_input = _write_ready_inputs(root)

        first = build_research_next_cycle_records_evidence_packet_31_payload(gate_input)
        second = build_research_next_cycle_records_evidence_packet_31_payload(gate_input)

        assert first == second
        assert first["phase"] == "31B"
        assert first["workflow"] == "Next Cycle Records Evidence Packet 31"
        assert first["records_evidence_packet_31_state"] == RECORDS_EVIDENCE_PACKET_31_READY_FOR_HUMAN_REVIEW
        ...
        assert first["summary"]["evidence_reference_count"] == 13
        assert first["queue_next_phase"]["phase"] == "31B"
...
def test_31B_writes_json_and_markdown_gate_31() -> None:
    root = _root()
    out_dir = root / "out"
    try:
        ...
        assert json_path.name == "research_next_cycle_records_evidence_packet_31.json"
        assert markdown_path.name == "research_next_cycle_records_evidence_packet_31.md"
        assert payload["records_evidence_packet_31_id"] == "31B-RESEARCH-NEXT-CYCLE-RECORDS-EVIDENCE-PACKET-31-2026-07-07"
        assert "31B Next Cycle Records Evidence Packet 31" in markdown
        assert "Final Records Evidence Packet 31 Summary" in markdown
...
def test_31B_classifies_blocked_for_missing_source_and_failed_safety_scanner() -> None:
    root = _root()
    try:
        gate_input = _write_ready_inputs(root)
        gate_input.records_review_gate_31_path.unlink()
        _write_json(
            gate_input.safety_scanner_path,
            {
                "workflow_id": "safety_scanner",
        ...
        markdown = render_research_next_cycle_records_evidence_packet_31_markdown(payload)

        assert payload["records_evidence_packet_31_state"] == BLOCKED_BY_SAFETY_GATE
        assert "records_review_gate_31" in {item["artifact_id"] for item in payload["missing_artifacts"]}
        assert "missing_records_review_gate_31" in {
...
def test_31B_rejects_secret_paths_unsafe_labels_and_execution_flags() -> None:
    root = _root()
    try:
        with pytest.raises(ValueError, match="secret files"):
            ResearchNextCycleRecordsEvidencePacket31Input(
                records_evidence_packet_31_id="31B-UNSAFE",
                records_evidence_packet_31_date="2026-07-07",
                generated_at_utc=FIXED_NOW.isoformat(),
        ...
        with pytest.raises(ValueError, match="unsafe|disallowed"):
            build_research_next_cycle_records_evidence_packet_31_payload(gate_input)
...
def test_31B_default_input_uses_phase_gate_id() -> None:
    gate_input = build_default_research_next_cycle_records_evidence_packet_31_input(now=FIXED_NOW)

    assert gate_input.records_evidence_packet_31_id == "31B-RESEARCH-NEXT-CYCLE-RECORDS-EVIDENCE-PACKET-31-2026-07-07"
    assert gate_input.records_evidence_packet_31_date == "2026-07-07"


def test_31B_cli_writes_next_cycle_records_evidence_packet_31() -> None:
    root = _root()
    out_dir = root / "out"
    try:
        gate_input = _write_ready_inputs(root)
        completed = subprocess.run(
            [
                sys.executable,
                "scripts/run_research_next_cycle_records_evidence_packet_31.py",
                "--out-dir",
                str(out_dir),
                "--records-evidence-packet-31-date",
                "2026-07-07",
        ...
        assert "JARVIS 31B NEXT CYCLE RECORDS EVIDENCE PACKET 31: COMPLETE" in completed.stdout
        assert "Records Evidence Packet 31 is read-only and records-only" in completed.stdout
        ...
        assert "Records Evidence Packet 31 records do not grant execution permission" in completed.stdout
        assert "LIVE TRADING: DISABLED" in completed.stdout
        assert (out_dir / "research_next_cycle_records_evidence_packet_31.json").exists()
        assert (out_dir / "research_next_cycle_records_evidence_packet_31.md").exists()
    finally:
        shutil.rmtree(root, ignore_errors=True)
