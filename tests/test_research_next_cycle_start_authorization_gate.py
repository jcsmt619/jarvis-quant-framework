...
from core.research_next_cycle_start_authorization_gate import (
    ...
    build_research_next_cycle_start_authorization_gate_payload,
    render_research_next_cycle_start_authorization_gate_markdown,
...
def _root() -> Path:
    return Path("reports/research_next_cycle_start_authorization_gate_tests") / uuid.uuid4().hex
...
def _packet_input(root: Path) -> ResearchNextCycleStartAuthorizationGateInput:
    return ResearchNextCycleStartAuthorizationGateInput(
        start_authorization_gate_id="29A-RESEARCH-NEXT-CYCLE-START-AUTHORIZATION-GATE-2026-07-07",
        start_authorization_gate_date="2026-07-07",
...
def _write_ready_inputs(root: Path) -> ResearchNextCycleStartAuthorizationGateInput:
    ...
    _write_json(
        ...
        [
            {
                "phase": "29A",
                "title": "Next Cycle Start Authorization Gate",
                "spec": "Build deterministic Start Authorization Gate records.",
            },
            {
                "phase": "28C",
                "title": "Next Cycle Records Review",
...
def test_29A_builds_deterministic_start_authorization_gate() -> None:
    root = _root()
    try:
        packet_input = _write_ready_inputs(root)

        first = build_research_next_cycle_start_authorization_gate_payload(packet_input)
        second = build_research_next_cycle_start_authorization_gate_payload(packet_input)

        assert first == second
        assert first["phase"] == "29A"
        assert first["workflow"] == "Next Cycle Start Authorization Gate"
        assert first["start_authorization_gate_state"] == START_AUTHORIZATION_READY_FOR_HUMAN_REVIEW
        ...
        assert first["queue_next_phase"]["phase"] == "29A"
...
def test_29A_writes_json_and_markdown_packet() -> None:
    root = _root()
    out_dir = root / "out"
    try:
        ...
        assert json_path.name == "research_next_cycle_start_authorization_gate.json"
        assert markdown_path.name == "research_next_cycle_start_authorization_gate.md"
        assert payload["start_authorization_gate_id"] == "29A-RESEARCH-NEXT-CYCLE-START-AUTHORIZATION-GATE-2026-07-07"
        assert "29A Next Cycle Start Authorization Gate" in markdown
        assert "Final Next-Cycle Start Authorization" in markdown
        ...
        assert "Acceptance State" in markdown
        assert "Dry-Run State" in markdown
        assert "Source States" in markdown
...
def test_29A_classifies_blocked_for_missing_source_and_failed_safety_scanner() -> None:
    root = _root()
    try:
        packet_input = _write_ready_inputs(root)
        ...
        markdown = render_research_next_cycle_start_authorization_gate_markdown(payload)

        assert payload["start_authorization_gate_state"] == BLOCKED_BY_SAFETY_GATE
...
def test_29A_rejects_secret_paths_unsafe_labels_and_execution_flags() -> None:
    root = _root()
    try:
        with pytest.raises(ValueError, match="secret files"):
            ResearchNextCycleStartAuthorizationGateInput(
                start_authorization_gate_id="29A-UNSAFE",
                start_authorization_gate_date="2026-07-07",
                generated_at_utc=FIXED_NOW.isoformat(),
        ...
        with pytest.raises(ValueError, match="unsafe research Next Cycle Start Preconditions Gate label"):
            build_research_next_cycle_start_authorization_gate_payload(packet_input)
...
def test_29A_default_input_uses_phase_packet_id() -> None:
    packet_input = build_default_research_next_cycle_start_authorization_gate_input(now=FIXED_NOW)

    assert packet_input.start_authorization_gate_id == "29A-RESEARCH-NEXT-CYCLE-START-AUTHORIZATION-GATE-2026-07-07"
    assert packet_input.start_authorization_gate_date == "2026-07-07"


def test_29A_cli_writes_next_cycle_start_authorization_gate() -> None:
    root = _root()
    out_dir = root / "out"
    try:
        packet_input = _write_ready_inputs(root)
        completed = subprocess.run(
            [
                sys.executable,
                "scripts/run_research_next_cycle_start_authorization_gate.py",
                "--out-dir",
                str(out_dir),
                "--start-authorization-gate-date",
                "2026-07-07",
        ...
        assert "JARVIS 29A NEXT CYCLE START AUTHORIZATION GATE: COMPLETE" in completed.stdout
        assert "Next-cycle start authorization gate is read-only and records-only" in completed.stdout
        ...
        assert (out_dir / "research_next_cycle_start_authorization_gate.json").exists()
        assert (out_dir / "research_next_cycle_start_authorization_gate.md").exists()
    finally:
        shutil.rmtree(root, ignore_errors=True)
