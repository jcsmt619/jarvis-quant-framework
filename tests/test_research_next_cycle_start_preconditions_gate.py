...
from core.research_next_cycle_start_preconditions_gate import (
    ...
    build_default_research_next_cycle_start_preconditions_gate_input,
    build_research_next_cycle_start_preconditions_gate_payload,
    render_research_next_cycle_start_preconditions_gate_markdown,
    write_research_next_cycle_start_preconditions_gate,
)
...
def _root() -> Path:
    return Path("reports/research_next_cycle_start_preconditions_gate_tests") / uuid.uuid4().hex
...
def _packet_input(root: Path) -> ResearchNextCycleStartPreconditionsGateInput:
    return ResearchNextCycleStartPreconditionsGateInput(
        start_preconditions_gate_id="28A-RESEARCH-NEXT-CYCLE-START-PRECONDITIONS-GATE-2026-07-07",
        start_preconditions_gate_date="2026-07-07",
...
def _write_ready_inputs(root: Path) -> ResearchNextCycleStartPreconditionsGateInput:
    ...
    payloads = {
        packet_input.operator_handoff_packet_path: {
            **_artifact_payload("27B", "Next Cycle Operator Handoff Packet", "operator_handoff_packet_date"),
            ...
            "launch_control_state": "LAUNCH_CONTROL_READY_FOR_HUMAN_REVIEW",
    ...
    _write_json(
        ...
        [
            {
                "phase": "28A",
                "title": "Next Cycle Start Preconditions Gate",
                "spec": "Build deterministic Start Preconditions Gate records.",
            },
            {
                "phase": "26C",
                "title": "Next Records-Only Research Report",
...
def test_28A_builds_deterministic_start_preconditions_gate() -> None:
    root = _root()
    try:
        packet_input = _write_ready_inputs(root)

        first = build_research_next_cycle_start_preconditions_gate_payload(packet_input)
        second = build_research_next_cycle_start_preconditions_gate_payload(packet_input)

        assert first == second
        assert first["phase"] == "28A"
        assert first["workflow"] == "Next Cycle Start Preconditions Gate"
        assert first["start_preconditions_gate_state"] == START_PRECONDITIONS_READY_FOR_HUMAN_REVIEW
        assert first["summary"]["source_artifact_count"] == 10
        assert first["summary"]["accepted_source_artifact_count"] == 10
        ...
        assert first["dry_run_state"] == "DRY_RUN_MANIFEST_READY_FOR_HUMAN_REVIEW"
        assert first["queue_next_phase"]["phase"] == "28A"
        ...
        assert first["start_preconditions_checklist"][0]["run_started"] is False
...
def test_28A_writes_json_and_markdown_gate() -> None:
    root = _root()
    out_dir = root / "out"
    try:
        json_path, markdown_path = write_research_next_cycle_start_preconditions_gate(
        ...
        assert json_path.name == "research_next_cycle_start_preconditions_gate.json"
        assert markdown_path.name == "research_next_cycle_start_preconditions_gate.md"
        assert payload["start_preconditions_gate_id"] == "28A-RESEARCH-NEXT-CYCLE-START-PRECONDITIONS-GATE-2026-07-07"
        assert "28A Next Cycle Start Preconditions Gate" in markdown
        assert "Final Next-Cycle Start Preconditions Summary" in markdown
        assert "Handoff State" in markdown
        assert "Dry-Run State" in markdown
        assert "Source Artifact Status" in markdown
        assert "Accepted Source Artifacts" in markdown
        ...
        assert "Start Preconditions Criteria" in markdown
        ...
        assert "Start Preconditions Checklist" in markdown
...
def test_28A_classifies_blocked_for_missing_gate_and_failed_safety_scanner() -> None:
    root = _root()
    try:
        packet_input = _write_ready_inputs(root)
        ...
        markdown = render_research_next_cycle_start_preconditions_gate_markdown(payload)

        assert payload["start_preconditions_gate_state"] == BLOCKED_BY_SAFETY_GATE
...
def test_28A_rejects_secret_paths_unsafe_labels_and_execution_flags() -> None:
    root = _root()
    try:
        with pytest.raises(ValueError, match="secret files"):
            ResearchNextCycleStartPreconditionsGateInput(
                start_preconditions_gate_id="28A-UNSAFE",
                start_preconditions_gate_date="2026-07-07",
                generated_at_utc=FIXED_NOW.isoformat(),
        ...
        with pytest.raises(ValueError, match="unsafe research Next Cycle Start Preconditions Gate label"):
            build_research_next_cycle_start_preconditions_gate_payload(packet_input)
...
def test_28A_default_input_uses_phase_gate_id() -> None:
    packet_input = build_default_research_next_cycle_start_preconditions_gate_input(now=FIXED_NOW)

    assert packet_input.start_preconditions_gate_id == "28A-RESEARCH-NEXT-CYCLE-START-PRECONDITIONS-GATE-2026-07-07"
    assert packet_input.start_preconditions_gate_date == "2026-07-07"


def test_28A_cli_writes_next_cycle_start_preconditions_gate() -> None:
    root = _root()
    out_dir = root / "out"
    try:
        packet_input = _write_ready_inputs(root)
        completed = subprocess.run(
            [
                sys.executable,
                "scripts/run_research_next_cycle_start_preconditions_gate.py",
                "--out-dir",
                str(out_dir),
                "--start-preconditions-gate-date",
                "2026-07-07",
        ...
        assert "JARVIS 28A NEXT CYCLE START PRECONDITIONS GATE: COMPLETE" in completed.stdout
        assert "Next-cycle start preconditions gate is read-only and records-only" in completed.stdout
        ...
        assert (out_dir / "research_next_cycle_start_preconditions_gate.json").exists()
        assert (out_dir / "research_next_cycle_start_preconditions_gate.md").exists()
    finally:
        shutil.rmtree(root, ignore_errors=True)
