...
from core.research_next_cycle_frozen_start_evidence_packet import (
    ...
    build_research_next_cycle_frozen_start_evidence_packet_payload,
    render_research_next_cycle_frozen_start_evidence_packet_markdown,
...
def _root() -> Path:
    return Path("reports/research_next_cycle_frozen_start_evidence_packet_tests") / uuid.uuid4().hex
...
def _packet_input(root: Path) -> ResearchNextCycleFrozenStartEvidencePacketInput:
    return ResearchNextCycleFrozenStartEvidencePacketInput(
        frozen_start_evidence_packet_id="30B-RESEARCH-NEXT-CYCLE-FROZEN-START-EVIDENCE-PACKET-2026-07-07",
        frozen_start_evidence_packet_date="2026-07-07",
...
def _write_ready_inputs(root: Path) -> ResearchNextCycleFrozenStartEvidencePacketInput:
    packet_input = _packet_input(root)
    payloads = {
        packet_input.frozen_start_review_gate_path: {
            **_artifact_payload("30A", "Next Cycle Frozen Start Review Gate", "frozen_start_review_gate_date"),
            ...
            "inert_command_hints": [
                {
                    "hint_id": "30A-HINT-30B",
                    "step_id": "30A-STEP-30B",
                    "label": HUMAN_REVIEW_REQUIRED,
                    "summary": "Review frozen start evidence.",
                    "command_hint": "python scripts/run_research_next_cycle_frozen_start_evidence_packet.py",
                    "would_run": False,
                    "executed": False,
                }
            ],
        },
        packet_input.frozen_launch_packet_path: {
        ...
        [
            {
                "phase": "30B",
                "title": "Next Cycle Frozen Start Evidence Packet",
                "spec": "Build deterministic frozen start evidence records.",
            }
        ],
    )
    return packet_input
...
def test_30B_builds_deterministic_frozen_start_evidence_packet() -> None:
    root = _root()
    try:
        packet_input = _write_ready_inputs(root)

        first = build_research_next_cycle_frozen_start_evidence_packet_payload(packet_input)
        second = build_research_next_cycle_frozen_start_evidence_packet_payload(packet_input)

        assert first == second
        assert first["phase"] == "30B"
        assert first["workflow"] == "Next Cycle Frozen Start Evidence Packet"
        assert first["frozen_start_evidence_packet_state"] == FROZEN_START_EVIDENCE_READY_FOR_HUMAN_REVIEW
        ...
        assert first["summary"]["evidence_reference_count"] == 11
        assert first["queue_next_phase"]["phase"] == "30B"
...
def test_30B_writes_json_and_markdown_packet() -> None:
    root = _root()
    out_dir = root / "out"
    try:
        ...
        assert json_path.name == "research_next_cycle_frozen_start_evidence_packet.json"
        assert markdown_path.name == "research_next_cycle_frozen_start_evidence_packet.md"
        assert payload["frozen_start_evidence_packet_id"] == "30B-RESEARCH-NEXT-CYCLE-FROZEN-START-EVIDENCE-PACKET-2026-07-07"
        assert "30B Next Cycle Frozen Start Evidence Packet" in markdown
        assert "Final Frozen Start Evidence Summary" in markdown
...
def test_30B_classifies_blocked_for_missing_source_and_failed_safety_scanner() -> None:
    root = _root()
    try:
        packet_input = _write_ready_inputs(root)
        packet_input.frozen_start_review_gate_path.unlink()
        _write_json(
            packet_input.safety_scanner_path,
            {
                "workflow_id": "safety_scanner",
        ...
        markdown = render_research_next_cycle_frozen_start_evidence_packet_markdown(payload)

        assert payload["frozen_start_evidence_packet_state"] == BLOCKED_BY_SAFETY_GATE
        assert "frozen_start_review_gate" in {item["artifact_id"] for item in payload["missing_artifacts"]}
        assert "missing_frozen_start_review_gate" in {
...
def test_30B_rejects_secret_paths_unsafe_labels_and_execution_flags() -> None:
    root = _root()
    try:
        with pytest.raises(ValueError, match="secret files"):
            ResearchNextCycleFrozenStartEvidencePacketInput(
                frozen_start_evidence_packet_id="30B-UNSAFE",
                frozen_start_evidence_packet_date="2026-07-07",
                generated_at_utc=FIXED_NOW.isoformat(),
                frozen_start_review_gate_path=Path(".env"),
            ).validate()

        packet_input = _write_ready_inputs(root)
        review_gate = json.loads(packet_input.frozen_start_review_gate_path.read_text(encoding="utf-8"))
        review_gate["label"] = "BUY" + "_NOW"
        _write_json(packet_input.frozen_start_review_gate_path, review_gate)
        with pytest.raises(ValueError, match="unsafe|disallowed"):
            build_research_next_cycle_frozen_start_evidence_packet_payload(packet_input)

        review_gate["label"] = HUMAN_REVIEW_REQUIRED
        review_gate["live_trading_" + "enabled"] = True
        _write_json(packet_input.frozen_start_review_gate_path, review_gate)
...
def test_30B_default_input_uses_phase_packet_id() -> None:
    packet_input = build_default_research_next_cycle_frozen_start_evidence_packet_input(now=FIXED_NOW)

    assert packet_input.frozen_start_evidence_packet_id == "30B-RESEARCH-NEXT-CYCLE-FROZEN-START-EVIDENCE-PACKET-2026-07-07"
    assert packet_input.frozen_start_evidence_packet_date == "2026-07-07"


def test_30B_cli_writes_next_cycle_frozen_start_evidence_packet() -> None:
    root = _root()
    out_dir = root / "out"
    try:
        packet_input = _write_ready_inputs(root)
        completed = subprocess.run(
            [
                sys.executable,
                "scripts/run_research_next_cycle_frozen_start_evidence_packet.py",
                "--out-dir",
                str(out_dir),
                "--frozen-start-evidence-packet-date",
                "2026-07-07",
                "--frozen-start-review-gate-path",
                str(packet_input.frozen_start_review_gate_path),
                "--frozen-launch-packet-path",
                str(packet_input.frozen_launch_packet_path),
        ...
        assert "JARVIS 30B NEXT CYCLE FROZEN START EVIDENCE PACKET: COMPLETE" in completed.stdout
        assert "Frozen start evidence packet is read-only and records-only" in completed.stdout
        assert "Evidence records do not grant execution permission or start any workflow" in completed.stdout
        ...
        assert (out_dir / "research_next_cycle_frozen_start_evidence_packet.json").exists()
        assert (out_dir / "research_next_cycle_frozen_start_evidence_packet.md").exists()
    finally:
        shutil.rmtree(root, ignore_errors=True)
