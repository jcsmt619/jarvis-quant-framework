...
from core.research_next_cycle_start_checklist_packet import (
    ...
    build_default_research_next_cycle_start_checklist_packet_input,
    build_research_next_cycle_start_checklist_packet_payload,
    render_research_next_cycle_start_checklist_packet_markdown,
    write_research_next_cycle_start_checklist_packet,
)
...
def _root() -> Path:
    return Path("reports/research_next_cycle_start_checklist_packet_tests") / uuid.uuid4().hex
...
def _packet_input(root: Path) -> ResearchNextCycleStartChecklistPacketInput:
    return ResearchNextCycleStartChecklistPacketInput(
        start_checklist_packet_id="28B-RESEARCH-NEXT-CYCLE-START-CHECKLIST-PACKET-2026-07-07",
        start_checklist_packet_date="2026-07-07",
...
def _write_ready_inputs(root: Path) -> ResearchNextCycleStartChecklistPacketInput:
    ...
    _write_json(
        ...
        [
            {
                "phase": "28B",
                "title": "Next Cycle Start Checklist Packet",
                "spec": "Build deterministic start checklist packet records.",
            },
            {
                "phase": "28C",
                "title": "Next Cycle Records Review",
...
def test_28B_builds_deterministic_start_checklist_packet() -> None:
    root = _root()
    try:
        packet_input = _write_ready_inputs(root)

        first = build_research_next_cycle_start_checklist_packet_payload(packet_input)
        second = build_research_next_cycle_start_checklist_packet_payload(packet_input)

        assert first == second
        assert first["phase"] == "28B"
        assert first["workflow"] == "Next Cycle Start Checklist Packet"
        assert first["start_checklist_packet_state"] == START_CHECKLIST_READY_FOR_HUMAN_REVIEW
        ...
        assert first["queue_next_phase"]["phase"] == "28B"
...
def test_28B_writes_json_and_markdown_packet() -> None:
    root = _root()
    out_dir = root / "out"
    try:
        json_path, markdown_path = write_research_next_cycle_start_checklist_packet(
        ...
        assert json_path.name == "research_next_cycle_start_checklist_packet.json"
        assert markdown_path.name == "research_next_cycle_start_checklist_packet.md"
        assert payload["start_checklist_packet_id"] == "28B-RESEARCH-NEXT-CYCLE-START-CHECKLIST-PACKET-2026-07-07"
        assert "28B Next Cycle Start Checklist Packet" in markdown
        assert "Final Next-Cycle Start Checklist" in markdown
...
def test_28B_classifies_blocked_for_missing_source_and_failed_safety_scanner() -> None:
    root = _root()
    try:
        packet_input = _write_ready_inputs(root)
        packet_input.launch_control_gate_path.unlink()
        _write_json(
            packet_input.safety_scanner_path,
            {
                "workflow_id": "safety_scanner",
        ...
        markdown = render_research_next_cycle_start_checklist_packet_markdown(payload)

        assert payload["start_checklist_packet_state"] == BLOCKED_BY_SAFETY_GATE
        assert "launch_control_gate" in {item["artifact_id"] for item in payload["missing_artifacts"]}
        assert "missing_launch_control_gate" in {
...
def test_28B_rejects_secret_paths_unsafe_labels_and_execution_flags() -> None:
    root = _root()
    try:
        with pytest.raises(ValueError, match="secret files"):
            ResearchNextCycleStartChecklistPacketInput(
                start_checklist_packet_id="28B-UNSAFE",
                start_checklist_packet_date="2026-07-07",
                generated_at_utc=FIXED_NOW.isoformat(),
                start_preconditions_gate_path=Path(".env"),
            ).validate()
        ...
        with pytest.raises(ValueError, match="unsafe research Next Cycle Start Preconditions Gate label"):
            build_research_next_cycle_start_checklist_packet_payload(packet_input)
...
def test_28B_default_input_uses_phase_packet_id() -> None:
    packet_input = build_default_research_next_cycle_start_checklist_packet_input(now=FIXED_NOW)

    assert packet_input.start_checklist_packet_id == "28B-RESEARCH-NEXT-CYCLE-START-CHECKLIST-PACKET-2026-07-07"
    assert packet_input.start_checklist_packet_date == "2026-07-07"


def test_28B_cli_writes_next_cycle_start_checklist_packet() -> None:
    root = _root()
    out_dir = root / "out"
    try:
        packet_input = _write_ready_inputs(root)
        completed = subprocess.run(
            [
                sys.executable,
                "scripts/run_research_next_cycle_start_checklist_packet.py",
                "--out-dir",
                str(out_dir),
                "--start-checklist-packet-date",
                "2026-07-07",
        ...
        assert "JARVIS 28B NEXT CYCLE START CHECKLIST PACKET: COMPLETE" in completed.stdout
        assert "Next-cycle start checklist packet is read-only and records-only" in completed.stdout
        ...
        assert (out_dir / "research_next_cycle_start_checklist_packet.json").exists()
        assert (out_dir / "research_next_cycle_start_checklist_packet.md").exists()
    finally:
        shutil.rmtree(root, ignore_errors=True)
