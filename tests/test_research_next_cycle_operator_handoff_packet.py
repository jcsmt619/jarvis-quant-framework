...
from core.research_next_cycle_operator_handoff_packet import (
    ...
    build_research_next_cycle_operator_handoff_packet_payload,
    render_research_next_cycle_operator_handoff_packet_markdown,
...
def _root() -> Path:
    return Path("reports/research_next_cycle_operator_handoff_packet_tests") / uuid.uuid4().hex
...
def _packet_input(root: Path) -> ResearchNextCycleOperatorHandoffPacketInput:
    return ResearchNextCycleOperatorHandoffPacketInput(
        operator_handoff_packet_id="27B-RESEARCH-NEXT-CYCLE-OPERATOR-HANDOFF-PACKET-2026-07-07",
        operator_handoff_packet_date="2026-07-07",
...
def _write_ready_inputs(root: Path) -> ResearchNextCycleOperatorHandoffPacketInput:
    ...
    _write_json(
        ...
        [
            {
                "phase": "27B",
                "title": "Next Cycle Operator Handoff Packet",
                "spec": "Build deterministic operator handoff packet records.",
            },
            {
                "phase": "26C",
                "title": "Next Records-Only Research Report",
...
def test_27B_builds_deterministic_operator_handoff_packet() -> None:
    root = _root()
    try:
        packet_input = _write_ready_inputs(root)

        first = build_research_next_cycle_operator_handoff_packet_payload(packet_input)
        second = build_research_next_cycle_operator_handoff_packet_payload(packet_input)

        assert first == second
        assert first["phase"] == "27B"
        assert first["workflow"] == "Next Cycle Operator Handoff Packet"
        assert first["operator_handoff_packet_state"] == OPERATOR_HANDOFF_READY_FOR_HUMAN_REVIEW
        assert first["summary"]["source_artifact_count"] == 8
        assert first["summary"]["accepted_source_artifact_count"] == 8
        ...
        assert first["queue_next_phase"]["phase"] == "27B"
        ...
        assert first["operator_handoff_checklist"][0]["run_started"] is False
...
def test_27B_writes_json_and_markdown_gate() -> None:
    root = _root()
    out_dir = root / "out"
    try:
        ...
        assert json_path.name == "research_next_cycle_operator_handoff_packet.json"
        assert markdown_path.name == "research_next_cycle_operator_handoff_packet.md"
        assert payload["operator_handoff_packet_id"] == "27B-RESEARCH-NEXT-CYCLE-OPERATOR-HANDOFF-PACKET-2026-07-07"
        assert "27B Next Cycle Operator Handoff Packet" in markdown
        assert "Final Next-Cycle Operator Handoff Summary" in markdown
        ...
        assert "Operator Handoff Criteria" in markdown
        ...
        assert "Operator Handoff Checklist" in markdown
...
def test_27B_classifies_blocked_for_missing_gate_and_failed_safety_scanner() -> None:
    root = _root()
    try:
        packet_input = _write_ready_inputs(root)
        ...
        markdown = render_research_next_cycle_operator_handoff_packet_markdown(payload)

        assert payload["operator_handoff_packet_state"] == BLOCKED_BY_SAFETY_GATE
...
def test_27B_rejects_secret_paths_unsafe_labels_and_execution_flags() -> None:
    root = _root()
    try:
        with pytest.raises(ValueError, match="secret files"):
            ResearchNextCycleOperatorHandoffPacketInput(
                operator_handoff_packet_id="27B-UNSAFE",
                operator_handoff_packet_date="2026-07-07",
                generated_at_utc=FIXED_NOW.isoformat(),
        ...
        with pytest.raises(ValueError, match="unsafe research Next Cycle Operator Handoff Packet label"):
            build_research_next_cycle_operator_handoff_packet_payload(packet_input)
...
def test_27B_default_input_uses_phase_gate_id() -> None:
    packet_input = build_default_research_next_cycle_operator_handoff_packet_input(now=FIXED_NOW)

    assert packet_input.operator_handoff_packet_id == "27B-RESEARCH-NEXT-CYCLE-OPERATOR-HANDOFF-PACKET-2026-07-07"
    assert packet_input.operator_handoff_packet_date == "2026-07-07"


def test_27B_cli_writes_next_cycle_operator_handoff_packet() -> None:
    root = _root()
    out_dir = root / "out"
    try:
        packet_input = _write_ready_inputs(root)
        completed = subprocess.run(
            [
                sys.executable,
                "scripts/run_research_next_cycle_operator_handoff_packet.py",
                "--out-dir",
                str(out_dir),
                "--operator-handoff-packet-date",
                "2026-07-07",
        ...
        assert "JARVIS 27B NEXT CYCLE OPERATOR HANDOFF PACKET: COMPLETE" in completed.stdout
        assert "Next-cycle operator handoff packet is read-only and records-only" in completed.stdout
        ...
        assert (out_dir / "research_next_cycle_operator_handoff_packet.json").exists()
        assert (out_dir / "research_next_cycle_operator_handoff_packet.md").exists()
    finally:
        shutil.rmtree(root, ignore_errors=True)
