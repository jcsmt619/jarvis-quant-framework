...
def write_approved_record(approvals_dir: Path) -> str:
    record = create_approval_record(
        ...
        ttl_minutes=10,
        source="test",
        note="paper arm bridge audit heartbeat test",
        now=fixed_now(),
    )
...
def test_orchestrator_writes_paper_arm_bridge_audit_event_default_state(capsys, tmp_path):
    ...
    code = orchestrator_script.run_local_autonomous_orchestrator(
        ...
        session_id="paper_arm_bridge_audit_default",
        max_cycles=1,
    ...
    assert code == 0
    assert "Paper arm bridge audit integration enabled: true" in output
    assert "Paper arm bridge heartbeat integration enabled: true" in output
    assert "Paper arm bridge audit event written: true" in output
    assert "orchestrator_paper_arm_bridge_state" in ledger_text
    assert "paper_arm_bridge_integrated=true" in ledger_text
    assert "paper_arm_requested=false" in ledger_text
...
def test_orchestrator_writes_paper_arm_bridge_audit_event_requested_but_disabled(capsys, tmp_path):
    approvals_dir = tmp_path / "approvals"
    ...
    code = orchestrator_script.run_local_autonomous_orchestrator(
        ...
        session_dir=tmp_path / "sessions",
        heartbeat_file=tmp_path / "heartbeat.json",
        session_id="paper_arm_bridge_audit_requested",
        max_cycles=1,
    ...
    assert code == 0
    assert "orchestrator_paper_arm_bridge_state" in ledger_text
    assert "paper_arm_requested=true" in ledger_text
    assert "paper_arm_confirmation_accepted=true" in ledger_text
    assert "approval_receipt_gate_allowed=true" in ledger_text
    assert "paper_arm_attempted=false" in ledger_text
    assert "paper_arm_enabled=false" in ledger_text
    assert "broker_order_call_performed=false" in ledger_text
    assert "live_trading_enabled=false" in ledger_text
    assert "BRIDGE_WIRED_BUT_EXECUTION_DISABLED_IN_PHASE_10C_17" in ledger_text
    assert "Broker order call performed: false" in output
    assert "LIVE TRADING: DISABLED" in output


def test_orchestrator_paper_arm_bridge_heartbeat_notes_default_state(capsys, tmp_path):
    def fake_runner(**kwargs):
        return 0
    ...
    code = orchestrator_script.run_local_autonomous_orchestrator(
        ...
        session_id="paper_arm_bridge_heartbeat_default",
        max_cycles=1,
        injected_cycle_runner=fake_runner,
        now=fixed_now(),
    )
...
def test_orchestrator_paper_arm_bridge_heartbeat_notes_requested_but_disabled(capsys, tmp_path):
    approvals_dir = tmp_path / "approvals"
    approval_id = write_approved_record(approvals_dir)
    ...
    code = orchestrator_script.run_local_autonomous_orchestrator(
        ...
        session_dir=tmp_path / "sessions",
        heartbeat_file=heartbeat_path,
        session_id="paper_arm_bridge_heartbeat_requested",
        max_cycles=1,
        approval_id=approval_id,
        enable_paper_arm=True,
        paper_arm_confirmation=PAPER_ARM_BRIDGE_CONFIRMATION,
    ...
    assert "paper_arm_confirmation_accepted=true" in notes
