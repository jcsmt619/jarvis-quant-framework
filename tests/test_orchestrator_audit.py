...
def test_orchestrator_writes_success_cycle_audit(capsys, tmp_path):
    ...
    events = [
        event
        for event in events
        if event.get("event_type") not in {"orchestrator_inbox_processor_state", "orchestrator_approval_receipt_state", "orchestrator_paper_arm_bridge_state", "orchestrator_paper_arm_drill_state"}
    ]
