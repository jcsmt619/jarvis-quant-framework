...
from core.research_next_cycle_operator_acceptance_gate import (
    ...
    build_research_next_cycle_operator_acceptance_gate_payload,
    render_research_next_cycle_operator_acceptance_gate_markdown,
...
def _root() -> Path:
    return Path("reports/research_next_cycle_operator_acceptance_gate_tests") / uuid.uuid4().hex
...
def _manifest_input(root: Path) -> ResearchNextCycleOperatorAcceptanceGateInput:
    return ResearchNextCycleOperatorAcceptanceGateInput(
        operator_acceptance_gate_id="26A-RESEARCH-NEXT-CYCLE-OPERATOR-ACCEPTANCE-GATE-2026-07-07",
        operator_acceptance_gate_date="2026-07-07",
...
def _write_ready_inputs(root: Path) -> ResearchNextCycleOperatorAcceptanceGateInput:
    manifest_input = _manifest_input(root)
    payloads = {
        manifest_input.dry_run_manifest_path: {
        ...
        manifest_input.archive_index_path: {
        ...
        manifest_input.operations_console_path: {
    ...
    _write_json(
        ...
        [
            {
                "phase": "26A",
                "title": "Next Cycle Operator Acceptance Gate",
                "spec": "Build deterministic operator acceptance gate records.",
            },
            {
                "phase": "25C",
                "title": "Next Records-Only Research Report",
...
def test_26A_builds_deterministic_OPERATOR_ACCEPTANCE_GATE() -> None:
    root = _root()
    try:
        manifest_input = _write_ready_inputs(root)

        first = build_research_next_cycle_operator_acceptance_gate_payload(manifest_input)
        second = build_research_next_cycle_operator_acceptance_gate_payload(manifest_input)

        assert first == second
        assert first["phase"] == "26A"
        assert first["workflow"] == "Next Cycle Operator Acceptance Gate"
        assert first["operator_acceptance_gate_state"] == ACCEPTANCE_READY_FOR_HUMAN_REVIEW
        ...
        assert first["queue_next_phase"]["phase"] == "25C"
...
def test_26A_writes_json_and_markdown_manifest() -> None:
    root = _root()
    out_dir = root / "out"
    try:
        ...
        assert json_path.name == "research_next_cycle_operator_acceptance_gate.json"
        assert markdown_path.name == "research_next_cycle_operator_acceptance_gate.md"
        assert payload["operator_acceptance_gate_id"] == "26A-RESEARCH-NEXT-CYCLE-OPERATOR-ACCEPTANCE-GATE-2026-07-07"
        assert "26A Next Cycle Operator Acceptance Gate" in markdown
...
def test_26A_classifies_blocked_for_missing_preflight_and_failed_safety_scanner() -> None:
    root = _root()
    try:
        manifest_input = _write_ready_inputs(root)
        ...
        markdown = render_research_next_cycle_operator_acceptance_gate_markdown(payload)

        assert payload["operator_acceptance_gate_state"] == BLOCKED_BY_SAFETY_GATE
...
def test_26A_rejects_secret_paths_unsafe_labels_and_execution_flags() -> None:
    root = _root()
    try:
        with pytest.raises(ValueError, match="secret files"):
            ResearchNextCycleOperatorAcceptanceGateInput(
                operator_acceptance_gate_id="26A-UNSAFE",
                operator_acceptance_gate_date="2026-07-07",
                generated_at_utc=FIXED_NOW.isoformat(),
        ...
        with pytest.raises(ValueError, match="unsafe research next cycle operator acceptance gate label"):
            build_research_next_cycle_operator_acceptance_gate_payload(manifest_input)
...
def test_26A_default_input_uses_phase_manifest_id() -> None:
    manifest_input = build_default_research_next_cycle_operator_acceptance_gate_input(now=FIXED_NOW)

    assert manifest_input.operator_acceptance_gate_id == "26A-RESEARCH-NEXT-CYCLE-OPERATOR-ACCEPTANCE-GATE-2026-07-07"
    assert manifest_input.operator_acceptance_gate_date == "2026-07-07"


def test_26A_cli_writes_next_cycle_OPERATOR_ACCEPTANCE_GATE() -> None:
    root = _root()
    out_dir = root / "out"
    try:
        manifest_input = _write_ready_inputs(root)
        completed = subprocess.run(
            [
                sys.executable,
                "scripts/run_research_next_cycle_operator_acceptance_gate.py",
                "--out-dir",
                str(out_dir),
                "--operator-acceptance-gate-date",
                "2026-07-07",
                "--dry-run-manifest-path",
                str(manifest_input.dry_run_manifest_path),
                ...
                "--archive-index-path",
                str(manifest_input.archive_index_path),
                "--operations-console-path",
                str(manifest_input.operations_console_path),
                "--report-index-path",
        ...
        assert "JARVIS 26A NEXT CYCLE OPERATOR ACCEPTANCE GATE: COMPLETE" in completed.stdout
        assert "Next-cycle operator acceptance gate is read-only and records-only" in completed.stdout
        ...
        assert (out_dir / "research_next_cycle_operator_acceptance_gate.json").exists()
        assert (out_dir / "research_next_cycle_operator_acceptance_gate.md").exists()
    finally:
        shutil.rmtree(root, ignore_errors=True)
