...
from core.research_next_cycle_records_review_gate_32 import (
    ...
    build_default_research_next_cycle_records_review_gate_32_input,
    build_research_next_cycle_records_review_gate_32_payload,
    render_research_next_cycle_records_review_gate_32_markdown,
    write_research_next_cycle_records_review_gate_32,
)
...
def _root() -> Path:
    return Path("reports/research_next_cycle_records_review_gate_32_tests") / uuid.uuid4().hex
...
def _gate_input(root: Path) -> ResearchNextCycleRecordsReviewGate32Input:
    return ResearchNextCycleRecordsReviewGate32Input(
        records_review_gate_32_id="32A-RESEARCH-NEXT-CYCLE-RECORDS-REVIEW-GATE-32-2026-07-07",
        records_review_gate_32_date="2026-07-07",
...
def _write_ready_inputs(root: Path) -> ResearchNextCycleRecordsReviewGate32Input:
    ...
    payloads = {
        ...
        gate_input.frozen_start_evidence_packet_path: {
            **_artifact_payload("30B", "Next Cycle Frozen Start Evidence Packet", "frozen_start_evidence_packet_date"),
            ...
            "inert_command_hints": [
                {
                    "hint_id": "30B-HINT-32A",
                    "label": HUMAN_REVIEW_REQUIRED,
                    "summary": "Review records gate.",
                    "command_hint": "python scripts/run_research_next_cycle_records_review_gate_32.py",
                    "would_run": False,
                    "executed": False,
                }
            ],
    ...
    _write_json(
        ...
        [
            {
                "phase": "32A",
                "title": "Next Cycle Records Review Gate 32",
...
def test_32A_builds_deterministic_records_review_gate_32() -> None:
    root = _root()
    try:
        gate_input = _write_ready_inputs(root)

        first = build_research_next_cycle_records_review_gate_32_payload(gate_input)
        second = build_research_next_cycle_records_review_gate_32_payload(gate_input)

        assert first == second
        assert first["phase"] == "32A"
        assert first["workflow"] == "Next Cycle Records Review Gate 32"
        ...
        assert first["summary"]["evidence_reference_count"] == 14
        assert first["queue_next_phase"]["phase"] == "32A"
...
def test_32A_writes_json_and_markdown_gate_31() -> None:
    root = _root()
    out_dir = root / "out"
    try:
        json_path, markdown_path = write_research_next_cycle_records_review_gate_32(
        ...
        assert json_path.name == "research_next_cycle_records_review_gate_32.json"
        assert markdown_path.name == "research_next_cycle_records_review_gate_32.md"
        assert payload["records_review_gate_32_id"] == "32A-RESEARCH-NEXT-CYCLE-RECORDS-REVIEW-GATE-32-2026-07-07"
        assert "32A Next Cycle Records Review Gate 32" in markdown
        assert "Final Records Review Gate 32 Summary" in markdown
...
def test_32A_classifies_blocked_for_missing_source_and_failed_safety_scanner() -> None:
    root = _root()
    try:
        gate_input = _write_ready_inputs(root)
        gate_input.records_evidence_packet_31_path.unlink()
        _write_json(
            gate_input.safety_scanner_path,
            {
                "workflow_id": "safety_scanner",
        ...
        markdown = render_research_next_cycle_records_review_gate_32_markdown(payload)

        assert payload["records_review_gate_32_state"] == BLOCKED_BY_SAFETY_GATE
        assert "records_evidence_packet_31" in {item["artifact_id"] for item in payload["missing_artifacts"]}
        assert "missing_records_evidence_packet_31" in {
...
def test_32A_rejects_secret_paths_unsafe_labels_and_execution_flags() -> None:
    root = _root()
    try:
        with pytest.raises(ValueError, match="secret files"):
            ResearchNextCycleRecordsReviewGate32Input(
                records_review_gate_32_id="32A-UNSAFE",
                records_review_gate_32_date="2026-07-07",
                generated_at_utc=FIXED_NOW.isoformat(),
        ...
        with pytest.raises(ValueError, match="unsafe|disallowed"):
            build_research_next_cycle_records_review_gate_32_payload(gate_input)
...
def test_32A_default_input_uses_phase_gate_id() -> None:
    gate_input = build_default_research_next_cycle_records_review_gate_32_input(now=FIXED_NOW)

    assert gate_input.records_review_gate_32_id == "32A-RESEARCH-NEXT-CYCLE-RECORDS-REVIEW-GATE-32-2026-07-07"
    assert gate_input.records_review_gate_32_date == "2026-07-07"


def test_32A_cli_writes_next_cycle_records_review_gate_32() -> None:
    root = _root()
    out_dir = root / "out"
    try:
        gate_input = _write_ready_inputs(root)
        completed = subprocess.run(
            [
                sys.executable,
                "scripts/run_research_next_cycle_records_review_gate_32.py",
                "--out-dir",
                str(out_dir),
                "--records-review-gate-32-date",
                "2026-07-07",
        ...
        assert "JARVIS 32A NEXT CYCLE RECORDS REVIEW GATE 32: COMPLETE" in completed.stdout
        assert "Records Review Gate 32 is read-only and records-only" in completed.stdout
        ...
        assert "Records Review Gate 32 records do not grant execution permission" in completed.stdout
        assert "LIVE TRADING: DISABLED" in completed.stdout
        assert (out_dir / "research_next_cycle_records_review_gate_32.json").exists()
        assert (out_dir / "research_next_cycle_records_review_gate_32.md").exists()
    finally:
        shutil.rmtree(root, ignore_errors=True)
