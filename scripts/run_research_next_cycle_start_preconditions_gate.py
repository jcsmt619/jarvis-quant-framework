...
def main() -> int:
    parser = argparse.ArgumentParser(description="Write the 28A next cycle start preconditions gate.")
    parser.add_argument("--out-dir", default=str(DEFAULT_RESEARCH_NEXT_CYCLE_START_PRECONDITIONS_GATE_DIR))
    parser.add_argument("--start-preconditions-gate-date", default=None, help="YYYY-MM-DD; defaults to today UTC.")
    ...
    start_preconditions_gate_date = (
        datetime.strptime(args.start_preconditions_gate_date, "%Y-%m-%d").date()
        if args.start_preconditions_gate_date
        else None
    )
    packet_input = build_default_research_next_cycle_start_preconditions_gate_input(
        start_preconditions_gate_date=start_preconditions_gate_date,
    ...
    print("JARVIS 28A NEXT CYCLE START PRECONDITIONS GATE: COMPLETE")
    print("RESEARCH_ONLY / MONITOR_ONLY / PAPER_ONLY / HUMAN_REVIEW_REQUIRED / BLOCKED_BY_SAFETY_GATE")
    print("Next-cycle start preconditions gate is read-only and records-only")
