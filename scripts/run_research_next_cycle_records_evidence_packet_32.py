...
def main() -> int:
    parser = argparse.ArgumentParser(description="Write the 32B next cycle records evidence packet 32.")
    parser.add_argument("--out-dir", default=str(DEFAULT_RESEARCH_NEXT_CYCLE_RECORDS_EVIDENCE_PACKET_32_DIR))
    parser.add_argument("--records-evidence-packet-32-date", default=None, help="YYYY-MM-DD; defaults to today UTC.")
    ...
    gate_date = (
        datetime.strptime(args.records_evidence_packet_32_date, "%Y-%m-%d").date()
        if args.records_evidence_packet_32_date
        else None
    )
    gate_input = build_default_research_next_cycle_records_evidence_packet_32_input(
        records_evidence_packet_32_date=gate_date,
    ...
    print("JARVIS 32B NEXT CYCLE RECORDS EVIDENCE PACKET 32: COMPLETE")
    print("RESEARCH_ONLY / MONITOR_ONLY / PAPER_ONLY / HUMAN_REVIEW_REQUIRED / BLOCKED_BY_SAFETY_GATE")
    print("Records Evidence Packet 32 is read-only and records-only")
    ...
    print("Records Evidence Packet 32 records do not grant execution permission")
