...
def main() -> int:
    parser = argparse.ArgumentParser(description="Write the 28B next cycle start checklist packet.")
    parser.add_argument("--out-dir", default=str(DEFAULT_RESEARCH_NEXT_CYCLE_START_CHECKLIST_PACKET_DIR))
    parser.add_argument("--start-checklist-packet-date", default=None, help="YYYY-MM-DD; defaults to today UTC.")
    ...
    start_checklist_packet_date = (
        datetime.strptime(args.start_checklist_packet_date, "%Y-%m-%d").date()
        if args.start_checklist_packet_date
        else None
    )
    packet_input = build_default_research_next_cycle_start_checklist_packet_input(
        start_checklist_packet_date=start_checklist_packet_date,
    ...
    print("JARVIS 28B NEXT CYCLE START CHECKLIST PACKET: COMPLETE")
    print("RESEARCH_ONLY / MONITOR_ONLY / PAPER_ONLY / HUMAN_REVIEW_REQUIRED / BLOCKED_BY_SAFETY_GATE")
    print("Next-cycle start checklist packet is read-only and records-only")
