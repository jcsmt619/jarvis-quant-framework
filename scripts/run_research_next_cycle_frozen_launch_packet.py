...
def main() -> int:
    parser = argparse.ArgumentParser(description="Write the 29B next cycle frozen launch packet.")
    parser.add_argument("--out-dir", default=str(DEFAULT_RESEARCH_NEXT_CYCLE_FROZEN_LAUNCH_PACKET_DIR))
    parser.add_argument("--frozen-launch-packet-date", default=None, help="YYYY-MM-DD; defaults to today UTC.")
    ...
    frozen_launch_packet_date = (
        datetime.strptime(args.frozen_launch_packet_date, "%Y-%m-%d").date()
        if args.frozen_launch_packet_date
        else None
    )
    packet_input = build_default_research_next_cycle_frozen_launch_packet_input(
        frozen_launch_packet_date=frozen_launch_packet_date,
    ...
    print("JARVIS 29B NEXT CYCLE FROZEN LAUNCH PACKET: COMPLETE")
    print("RESEARCH_ONLY / MONITOR_ONLY / PAPER_ONLY / HUMAN_REVIEW_REQUIRED / BLOCKED_BY_SAFETY_GATE")
    print("Frozen launch packet is read-only and records-only")
    ...
    print("Frozen launch records do not grant execution permission")
