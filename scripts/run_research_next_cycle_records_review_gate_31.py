...
def main() -> int:
    parser = argparse.ArgumentParser(description="Write the 31A next cycle records review gate 31.")
    parser.add_argument("--out-dir", default=str(DEFAULT_RESEARCH_NEXT_CYCLE_RECORDS_REVIEW_GATE_31_DIR))
    parser.add_argument("--records-review-gate-31-date", default=None, help="YYYY-MM-DD; defaults to today UTC.")
    ...
    gate_date = (
        datetime.strptime(args.records_review_gate_31_date, "%Y-%m-%d").date()
        if args.records_review_gate_31_date
        else None
    )
    gate_input = build_default_research_next_cycle_records_review_gate_31_input(
        records_review_gate_31_date=gate_date,
    ...
    print("JARVIS 31A NEXT CYCLE RECORDS REVIEW GATE 31: COMPLETE")
    print("RESEARCH_ONLY / MONITOR_ONLY / PAPER_ONLY / HUMAN_REVIEW_REQUIRED / BLOCKED_BY_SAFETY_GATE")
    print("Records Review Gate 31 is read-only and records-only")
    ...
    print("Records Review Gate 31 records do not grant execution permission")
