...
def main() -> int:
    parser = argparse.ArgumentParser(description="Write the 22B operator signoff packet.")
    parser.add_argument("--out-dir", default=str(DEFAULT_OPERATOR_SIGNOFF_PACKET_DIR))
    parser.add_argument("--signoff-date", default=None, help="YYYY-MM-DD; defaults to today UTC.")
    ...
    signoff_date = (
        datetime.strptime(args.signoff_date, "%Y-%m-%d").date()
        if args.signoff_date
        else None
    )
    signoff_input = build_default_operator_signoff_packet_input(
    ...
    signoff_input = signoff_input.__class__(
        **{
            **signoff_input.__dict__,
            **{
                key: Path(value)
    ...
    print("JARVIS 22B OPERATOR SIGNOFF PACKET: COMPLETE")
    print("RESEARCH_ONLY / MONITOR_ONLY / PAPER_ONLY / HUMAN_REVIEW_REQUIRED / BLOCKED_BY_SAFETY_GATE")
    print("Signoff is records-only; it does not enable live trading, broker routing, broker calls, order execution, or automatic actions")
