...
from core.research_next_cycle_records_evidence_packet_33 import (
    DEFAULT_RESEARCH_NEXT_CYCLE_RECORDS_EVIDENCE_PACKET_33_DIR,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Write the 33B next cycle Records Evidence Packet 33.")
    parser.add_argument("--out-dir", default=str(DEFAULT_RESEARCH_NEXT_CYCLE_RECORDS_EVIDENCE_PACKET_33_DIR))
    parser.add_argument("--records-evidence-packet-33-date", default=None, help="YYYY-MM-DD; defaults to today UTC.")
    parser.add_argument("--records-review-gate-33-path", default=None)
    parser.add_argument("--records-evidence-packet-32-path", default=None)
    parser.add_argument("--records-review-gate-32-path", default=None)
    parser.add_argument("--records-evidence-packet-31-path", default=None)
    ...
    gate_date = (
        datetime.strptime(args.records_evidence_packet_33_date, "%Y-%m-%d").date()
        if args.records_evidence_packet_33_date
        else None
    )
    gate_input = build_default_research_next_cycle_records_evidence_packet_33_input(
        records_evidence_packet_33_date=gate_date,
        now=datetime.now(tz=UTC),
    )
    overrides = {
        "records_review_gate_33_path": args.records_review_gate_33_path,
        "records_evidence_packet_32_path": args.records_evidence_packet_32_path,
        "records_review_gate_32_path": args.records_review_gate_32_path,
        "records_evidence_packet_31_path": args.records_evidence_packet_31_path,
    ...
    print("JARVIS 33B NEXT CYCLE RECORDS EVIDENCE PACKET 33: COMPLETE")
    print("RESEARCH_ONLY / MONITOR_ONLY / PAPER_ONLY / HUMAN_REVIEW_REQUIRED / BLOCKED_BY_SAFETY_GATE")
    print("Records Evidence Packet 33 is read-only and records-only")
    ...
    print("Records Evidence Packet 33 records do not grant execution permission")
