...
def main() -> int:
    parser = argparse.ArgumentParser(description="Write the 26B next cycle acceptance packet.")
    parser.add_argument("--out-dir", default=str(DEFAULT_RESEARCH_NEXT_CYCLE_ACCEPTANCE_PACKET_DIR))
    parser.add_argument("--acceptance-packet-date", default=None, help="YYYY-MM-DD; defaults to today UTC.")
    ...
    acceptance_packet_date = (
        datetime.strptime(args.acceptance_packet_date, "%Y-%m-%d").date()
        if args.acceptance_packet_date
        else None
    )
    packet_input = build_default_research_next_cycle_acceptance_packet_input(
        acceptance_packet_date=acceptance_packet_date,
    ...
    json_path, markdown_path = write_research_next_cycle_acceptance_packet(packet_input, out_dir=Path(args.out_dir))

    print("JARVIS 26B NEXT CYCLE ACCEPTANCE PACKET: COMPLETE")
    print("RESEARCH_ONLY / MONITOR_ONLY / PAPER_ONLY / HUMAN_REVIEW_REQUIRED / BLOCKED_BY_SAFETY_GATE")
    print("Next-cycle acceptance packet is read-only and records-only")
