...
DEFAULT_RESEARCH_NEXT_CYCLE_OPERATOR_HANDOFF_PACKET_DIR = Path("reports/research_next_cycle_operator_handoff_packet")
RESEARCH_NEXT_CYCLE_OPERATOR_HANDOFF_PACKET_JSON = "research_next_cycle_operator_handoff_packet.json"
RESEARCH_NEXT_CYCLE_OPERATOR_HANDOFF_PACKET_MARKDOWN = "research_next_cycle_operator_handoff_packet.md"

OPERATOR_HANDOFF_READY_FOR_HUMAN_REVIEW = "OPERATOR_HANDOFF_READY_FOR_HUMAN_REVIEW"
NEEDS_OPERATOR_REVIEW = "NEEDS_OPERATOR_REVIEW"

SAFE_OPERATOR_HANDOFF_LABELS = (
    RESEARCH_ONLY,
    MONITOR_ONLY,
    PAPER_ONLY,
    HUMAN_REVIEW_REQUIRED,
...
OPERATOR_HANDOFF_PACKET_STATES = (
    OPERATOR_HANDOFF_READY_FOR_HUMAN_REVIEW,
    NEEDS_OPERATOR_REVIEW,
    BLOCKED_BY_SAFETY_GATE,
)
DISALLOWED_OPERATOR_HANDOFF_LABELS = tuple(
    verb + suffix
    for verb, suffix in (
        ("BUY", "_NOW"),
        ("SELL", "_NOW"),
...
class ResearchNextCycleOperatorHandoffPacketInput:
    operator_handoff_packet_id: str
    operator_handoff_packet_date: str
    ...
    def validate(self) -> None:
        for field_name in ("operator_handoff_packet_id", "operator_handoff_packet_date", "generated_at_utc"):
            value = getattr(self, field_name)
            if not isinstance(value, str) or not value.strip():
                raise ValueError(f"research Next Cycle Operator Handoff Packet requires {field_name}")
        _parse_iso_date("operator_handoff_packet_date", self.operator_handoff_packet_date)
...
def build_default_research_next_cycle_operator_handoff_packet_input(
    *,
    operator_handoff_packet_date: date | None = None,
    now: datetime | None = None,
) -> ResearchNextCycleOperatorHandoffPacketInput:
    generated = now or datetime.now(tz=UTC)
    day = operator_handoff_packet_date or generated.date()
    return ResearchNextCycleOperatorHandoffPacketInput(
        operator_handoff_packet_id=f"27B-RESEARCH-NEXT-CYCLE-OPERATOR-HANDOFF-PACKET-{day.isoformat()}",
        operator_handoff_packet_date=day.isoformat(),
        generated_at_utc=generated.isoformat(),
    )


def build_research_next_cycle_operator_handoff_packet_payload(
    packet_input: ResearchNextCycleOperatorHandoffPacketInput,
) -> dict[str, Any]:
    packet_input.validate()
    packet_day = datetime.strptime(packet_input.operator_handoff_packet_date, "%Y-%m-%d").date()
    ...
    operator_handoff_criteria = _operator_handoff_criteria(
    ...
    operator_handoff_packet_state = _operator_handoff_packet_state(
    ...
    operator_handoff_checklist = _operator_handoff_checklist(
    ...
    payload = {
        "phase": "27B",
        "workflow": "Next Cycle Operator Handoff Packet",
        "operator_handoff_packet_id": packet_input.operator_handoff_packet_id,
        "operator_handoff_packet_date": packet_input.operator_handoff_packet_date,
        "generated_at_utc": packet_input.generated_at_utc,
        "acceptance_state": acceptance_state,
        "launch_control_state": launch_control_state,
        "operator_handoff_packet_state": operator_handoff_packet_state,
        "safety_boundary": _safety_boundary(),
        "required_labels": [*SAFE_OPERATOR_HANDOFF_LABELS, "LIVE TRADING: DISABLED"],
        "summary": {
            ...
            "operator_handoff_criteria_count": len(operator_handoff_criteria),
            ...
            "operator_handoff_checklist_count": len(operator_handoff_checklist),
            "acceptance_state": acceptance_state,
            "launch_control_state": launch_control_state,
        ...
        "operator_review_items": operator_review_items,
        "safety_findings": safety_findings,
        "operator_handoff_criteria": operator_handoff_criteria,
        "unresolved_items": unresolved_items,
        ...
        "queue_status": queue_status,
        "safety_scanner_status": safety_scanner_status,
        "final_next_cycle_operator_handoff_summary": _final_summary(
            operator_handoff_packet_state=operator_handoff_packet_state,
            launch_control_state=launch_control_state,
    ...
    _validate_json_value("research_next_cycle_operator_handoff_packet_payload", payload)
    return _normalize_json_value(payload)


def write_research_next_cycle_operator_handoff_packet(
    packet_input: ResearchNextCycleOperatorHandoffPacketInput,
    *,
    out_dir: Path = DEFAULT_RESEARCH_NEXT_CYCLE_OPERATOR_HANDOFF_PACKET_DIR,
) -> tuple[Path, Path]:
    payload = build_research_next_cycle_operator_handoff_packet_payload(packet_input)
    out_dir.mkdir(parents=True, exist_ok=True)
    json_path = out_dir / RESEARCH_NEXT_CYCLE_OPERATOR_HANDOFF_PACKET_JSON
    markdown_path = out_dir / RESEARCH_NEXT_CYCLE_OPERATOR_HANDOFF_PACKET_MARKDOWN
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    markdown_path.write_text(render_research_next_cycle_operator_handoff_packet_markdown(payload), encoding="utf-8")
    return json_path, markdown_path


def render_research_next_cycle_operator_handoff_packet_markdown(payload: dict[str, Any]) -> str:
    _validate_json_value("research_next_cycle_operator_handoff_packet_payload", payload)
    lines = [
        "# 27B Next Cycle Operator Handoff Packet",
        "",
        f"Operator Handoff Packet ID: {payload['operator_handoff_packet_id']}",
        f"Operator Handoff Packet Date: {payload['operator_handoff_packet_date']}",
        f"Generated: {payload['generated_at_utc']}",
        f"Acceptance State: {payload['acceptance_state']}",
        f"Launch-Control State: {payload['launch_control_state']}",
        f"Operator Handoff Packet State: {payload['operator_handoff_packet_state']}",
        "",
        "Status: RESEARCH_ONLY / MONITOR_ONLY / PAPER_ONLY / HUMAN_REVIEW_REQUIRED / BLOCKED_BY_SAFETY_GATE.",
        "next-cycle operator handoff packets are read-only and records-only.",
        ...
        "",
        "## Final Next-Cycle Operator Handoff Summary",
        "",
        payload["final_next_cycle_operator_handoff_summary"],
        "",
        "## Summary",
        "",
        ...
        _summary_line("Operator Handoff Criteria", payload["summary"]["operator_handoff_criteria_count"]),
        ...
        _summary_line("Operator handoff checklist items", payload["summary"]["operator_handoff_checklist_count"]),
        "",
    ]
    ...
    lines.extend(_section("Operator Handoff Criteria", payload["operator_handoff_criteria"]))
    ...
    lines.extend(_section("Operator Handoff Checklist", payload["operator_handoff_checklist"]))
    ...
    lines.extend(
        [
            "## Safety Boundary",
            "",
            "- RESEARCH_ONLY operator handoff packet generation only.",
...
def _source_artifacts(packet_input: ResearchNextCycleOperatorHandoffPacketInput) -> list[dict[str, Any]]:
    artifact_paths = (
...
def _planned_dry_run_steps(
    ...
    if isinstance(source_steps, list):
        for index, item in enumerate(source_steps, start=1):
            ...
            steps.append(
                {
                    "step_id": f"27B-DRY-RUN-STEP-{phase}",
    ...
    if not steps:
        ...
        if isinstance(next_phase, dict):
            steps.append(
                {
                    "step_id": f"27B-DRY-RUN-STEP-{next_phase.get('phase') or 'NEXT'}",
                    "source_step_id": None,
...
def _command_hints(
    ...
    if not hints:
        for step in planned_steps:
            hints.append(
                {
                    "hint_id": f"27B-HINT-{step['step_id']}",
                    "source_hint_id": None,
...
def _skipped_steps(planned_steps: list[dict[str, Any]], missing_artifacts: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    return _dedupe_by_id(
        [
            {
                "skipped_step_id": f"27B-SKIPPED-{step['step_id']}",
                "step_id": step["step_id"],
                "label": label,
                "status": "skipped_by_records_only_gate",
                "summary": "Step is recorded for operator handoff review only and is not run by 27B.",
                "would_run": False,
                "run_started": False,
                "executed": False,
            }
...
def _required_human_review_actions(
    *,
    ...
    actions = [
        _action("27B-HUMAN-REVIEW-SAFETY-BOUNDARY", "Confirm 27B remains records-only and LIVE TRADING: DISABLED."),
        _action("27B-HUMAN-REVIEW-LAUNCH-CONTROL-STATE", "Review Operator Handoff Packet State before any separate future workflow is considered."),
    ]
    if blocked_prerequisites:
        actions.append(_action("27B-HUMAN-REVIEW-BLOCKED-PREREQUISITES", f"Review {len(blocked_prerequisites)} blocked prerequisites."))
    if required_refreshed_artifacts:
        actions.append(_action("27B-HUMAN-REVIEW-REFRESHED-ARTIFACTS", f"Refresh or explicitly accept {len(required_refreshed_artifacts)} artifacts."))
    if operator_review_items:
        actions.append(_action("27B-HUMAN-REVIEW-OPERATOR-ITEMS", f"Review {len(operator_review_items)} operator review items."))
    if safety_findings:
        actions.append(_action("27B-HUMAN-REVIEW-SAFETY-FINDINGS", f"Resolve {len(safety_findings)} safety findings."))
    if unresolved_items:
        actions.append(_action("27B-HUMAN-REVIEW-UNRESOLVED-ITEMS", f"Resolve or accept {len(unresolved_items)} unresolved items."))
    return _dedupe_by_id(actions, "action_id")


def _operator_handoff_criteria(
    *,
    planned_steps: list[dict[str, Any]],
    ...
    return _dedupe_by_id(
        [
            _criterion("27B-ACCEPT-RECORDS-ONLY", "27B produces deterministic JSON and Markdown records only.", RESEARCH_ONLY),
            _criterion("27B-ACCEPT-NO-WORKFLOW-START", "The next cycle is not run and no workflow is started.", HUMAN_REVIEW_REQUIRED),
            _criterion("27B-ACCEPT-INERT-COMMAND-HINTS", f"{len(command_hints)} command hints are inert records with executed=false.", HUMAN_REVIEW_REQUIRED),
            _criterion("27B-ACCEPT-PLANNED-DRY-RUN-STEPS", f"{len(planned_steps)} planned dry-run steps are recorded with would_run=false.", PAPER_ONLY),
            _criterion("27B-ACCEPT-SKIPPED-STEPS", f"{len(skipped_steps)} steps are explicitly skipped by the records-only gate.", PAPER_ONLY),
            _criterion("27B-ACCEPT-BLOCKED-PREREQUISITES", f"Blocked prerequisite count is {len(blocked_prerequisites)} and remains blocked.", BLOCKED_BY_SAFETY_GATE if blocked_prerequisites else HUMAN_REVIEW_REQUIRED),
            _criterion("27B-ACCEPT-REFRESHED-ARTIFACTS", f"Required refreshed artifact count is {len(required_refreshed_artifacts)}.", HUMAN_REVIEW_REQUIRED),
            _criterion("27B-ACCEPT-OPERATOR-REVIEW", f"Operator review item count is {len(operator_review_items)}.", HUMAN_REVIEW_REQUIRED),
            _criterion("27B-ACCEPT-SAFETY-FINDINGS", f"Safety finding count is {len(safety_findings)}.", BLOCKED_BY_SAFETY_GATE if safety_findings else HUMAN_REVIEW_REQUIRED),
            _criterion("27B-ACCEPT-UNRESOLVED-ITEMS", f"Unresolved item count is {len(unresolved_items)}.", HUMAN_REVIEW_REQUIRED),
            _criterion("27B-ACCEPT-LIVE-TRADING-DISABLED", "LIVE TRADING: DISABLED is present and no broker/order path is created.", HUMAN_REVIEW_REQUIRED),
        ],
        "criterion_id",
    )


def _operator_handoff_packet_state(
    *,
    missing_artifacts: list[dict[str, Any]],
    stale_artifacts: list[dict[str, Any]],
    safety_findings: list[dict[str, Any]],
    ...
    return OPERATOR_HANDOFF_READY_FOR_HUMAN_REVIEW
...
def _queue_next_phase(queue_status: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(next_phase, dict):
        return {
            ...
            "summary": "No queue next phase was available to 27B.",
            "run_started": False,
        }
    value = {
        "workflow_id": "queue_next_phase",
        ...
        "summary": next_phase.get("summary") or "Queue next phase recorded for operator handoff review only.",
        "run_started": False,
    }
...
def _queue_status(path: Path) -> dict[str, Any]:
    ...
    value = {
        ...
        "label": HUMAN_REVIEW_REQUIRED,
        "status": "read_only",
        "summary": "Master plan queue read for 27B operator handoff packet context only.",
        "path": path.as_posix(),
...
def _safety_scanner_status(path: Path) -> dict[str, Any]:
    ...
    if payload is None:
        return {
            ...
            "label": HUMAN_REVIEW_REQUIRED,
            "status": "not_run",
            "summary": "Safety scanner status was not supplied to 27B.",
            "path": path.as_posix(),
            "finding_count": 0,
            "passed": None,
            "findings": [],
    ...
    value = {
        ...
        "summary": payload.get("summary", "Safety scanner status supplied to 27B."),
        "path": path.as_posix(),
        ...
        accepted.append(
            {
                ...
                "name": item["name"],
                "label": item.get("label", HUMAN_REVIEW_REQUIRED),
                "status": "accepted_for_handoff_record",
                "summary": f"{item['name']} is accepted as read-only source context for 27B.",
                "path": item["path"],
...
def _operator_handoff_checklist(
    *,
    ...
    items = [
        _checklist("27B-CHECK-LAUNCH-CONTROL", f"Launch-control state recorded as {launch_control_state}.", HUMAN_REVIEW_REQUIRED),
        _checklist("27B-CHECK-SOURCE-ARTIFACTS", f"{len(accepted_source_artifacts)} source artifacts accepted for handoff context.", HUMAN_REVIEW_REQUIRED),
        _checklist("27B-CHECK-RECORDS-ONLY-STEPS", f"{len(planned_steps)} planned records-only steps are listed and not started.", PAPER_ONLY),
        _checklist("27B-CHECK-INERT-COMMAND-HINTS", f"{len(command_hints)} inert command hints are listed with executed=false.", HUMAN_REVIEW_REQUIRED),
        _checklist("27B-CHECK-BLOCKED-PREREQUISITES", f"{len(blocked_prerequisites)} blocked prerequisites remain blocked.", BLOCKED_BY_SAFETY_GATE if blocked_prerequisites else HUMAN_REVIEW_REQUIRED),
        _checklist("27B-CHECK-UNRESOLVED-ITEMS", f"{len(unresolved_items)} unresolved review items require human review or acceptance.", HUMAN_REVIEW_REQUIRED),
        _checklist("27B-CHECK-REFRESHED-ARTIFACTS", f"{len(required_refreshed_artifacts)} refreshed artifacts are required or explicitly accepted.", HUMAN_REVIEW_REQUIRED),
        _checklist("27B-CHECK-SAFETY-FINDINGS", f"{len(safety_findings)} safety findings are recorded.", BLOCKED_BY_SAFETY_GATE if safety_findings else HUMAN_REVIEW_REQUIRED),
        _checklist("27B-CHECK-LIVE-TRADING-DISABLED", "LIVE TRADING: DISABLED; no execution permission, broker routing, broker call, or order path is created.", HUMAN_REVIEW_REQUIRED),
    ]
...
def _final_summary(
    *,
    operator_handoff_packet_state: str,
    launch_control_state: str,
    ...
    operator_review_items: list[dict[str, Any]],
    safety_findings: list[dict[str, Any]],
    operator_handoff_criteria: list[dict[str, Any]],
    unresolved_items: list[dict[str, Any]],
    required_human_review_actions: list[dict[str, Any]],
    operator_handoff_checklist: list[dict[str, Any]],
) -> str:
    present = len([item for item in source_artifacts if item["status"] == "present"])
    return (
        f"Next-cycle Operator Handoff Packet State is {operator_handoff_packet_state}. "
        ...
        f"Operator Handoff Criteria: {len(operator_handoff_criteria)}. "
        f"Unresolved items: {len(unresolved_items)}. "
        f"Required human-review actions: {len(required_human_review_actions)}. "
        f"Operator handoff checklist items: {len(operator_handoff_checklist)}. "
        "Safety boundary confirmed: 27B is read-only and records-only and does not execute commands, run the next cycle, mutate artifacts, delete artifacts, create broker actions, create trade instructions, enable live trading, grant execution permissions, route broker orders, call broker endpoints, or submit any order path. "
        "LIVE TRADING: DISABLED."
    )
...
def _checklist(checklist_item_id: str, summary: str, label: str) -> dict[str, Any]:
    return {
        ...
        "label": label,
        "status": "handoff_review_required",
        "summary": summary,
        "records_only": True,
...
def _validate_gate_path(path: Path) -> None:
    if path.name in SECRET_FILE_NAMES:
        raise ValueError("research Next Cycle Operator Handoff Packet cannot target secret files")
    lowered_parts = [part.lower() for part in path.parts]
    if any(marker in part for part in lowered_parts for marker in SECRET_PATH_MARKERS):
        raise ValueError("research Next Cycle Operator Handoff Packet cannot target credential or secret paths")
    if path.is_absolute():
        raise ValueError("research Next Cycle Operator Handoff Packet paths must be repo-relative")
    if ".." in path.parts:
        raise ValueError("research Next Cycle Operator Handoff Packet paths cannot traverse parent directories")
...
def _validate_json_value(field_name: str, value: Any) -> None:
    if isinstance(value, dict):
        ...
        if label is not None and label not in SAFE_OPERATOR_HANDOFF_LABELS:
            raise ValueError(f"unsafe research Next Cycle Operator Handoff Packet label: {label}")
        state = value.get("operator_handoff_packet_state")
        if state is not None and state not in OPERATOR_HANDOFF_PACKET_STATES:
            raise ValueError(f"invalid research Next Cycle Operator Handoff Packet state: {state}")
        for unsafe_field in UNSAFE_TRUE_FIELDS:
            if value.get(unsafe_field) is True:
                raise ValueError(f"research Next Cycle Operator Handoff Packet cannot set {unsafe_field}")
        for mutation_field in ("artifact_delete_performed", "artifact_mutation_performed", "commands_executed"):
            if value.get(mutation_field) is True:
                raise ValueError(f"research Next Cycle Operator Handoff Packet cannot set {mutation_field}")
    ...
    if isinstance(value, str):
        if value in DISALLOWED_OPERATOR_HANDOFF_LABELS:
            raise ValueError(f"disallowed research Next Cycle Operator Handoff Packet text: {value}")
        return
