...
DEFAULT_RESEARCH_NEXT_CYCLE_OPERATOR_ACCEPTANCE_GATE_DIR = Path("reports/research_next_cycle_operator_acceptance_gate")
RESEARCH_NEXT_CYCLE_OPERATOR_ACCEPTANCE_GATE_JSON = "research_next_cycle_operator_acceptance_gate.json"
RESEARCH_NEXT_CYCLE_OPERATOR_ACCEPTANCE_GATE_MARKDOWN = "research_next_cycle_operator_acceptance_gate.md"

ACCEPTANCE_READY_FOR_HUMAN_REVIEW = "ACCEPTANCE_READY_FOR_HUMAN_REVIEW"
...
ACCEPTANCE_GATE_STATES = (
    ACCEPTANCE_READY_FOR_HUMAN_REVIEW,
...
class ResearchNextCycleOperatorAcceptanceGateInput:
    operator_acceptance_gate_id: str
    operator_acceptance_gate_date: str
    ...
    def validate(self) -> None:
        for field_name in ("operator_acceptance_gate_id", "operator_acceptance_gate_date", "generated_at_utc"):
            value = getattr(self, field_name)
            if not isinstance(value, str) or not value.strip():
                raise ValueError(f"research next cycle operator acceptance gate requires {field_name}")
        _parse_iso_date("operator_acceptance_gate_date", self.operator_acceptance_gate_date)
...
def build_default_research_next_cycle_operator_acceptance_gate_input(
    *,
    operator_acceptance_gate_date: date | None = None,
    now: datetime | None = None,
) -> ResearchNextCycleOperatorAcceptanceGateInput:
    generated = now or datetime.now(tz=UTC)
    day = operator_acceptance_gate_date or generated.date()
    return ResearchNextCycleOperatorAcceptanceGateInput(
        operator_acceptance_gate_id=f"26A-RESEARCH-NEXT-CYCLE-OPERATOR-ACCEPTANCE-GATE-{day.isoformat()}",
        operator_acceptance_gate_date=day.isoformat(),
        generated_at_utc=generated.isoformat(),
    )


def build_research_next_cycle_operator_acceptance_gate_payload(
    manifest_input: ResearchNextCycleOperatorAcceptanceGateInput,
) -> dict[str, Any]:
    manifest_input.validate()
    manifest_day = datetime.strptime(manifest_input.operator_acceptance_gate_date, "%Y-%m-%d").date()
    ...
    queue_next_phase = _queue_next_phase(queue_status, source_payloads.get("dry_run_manifest", {}))
    operator_acceptance_gate_state = _operator_acceptance_gate_state(
        missing_artifacts=missing_artifacts,
        stale_artifacts=stale_artifacts,
        safety_findings=safety_findings,
        blocked_prerequisites=blocked_prerequisites,
    ...
    payload = {
        "phase": "26A",
        "workflow": "Next Cycle Operator Acceptance Gate",
        "operator_acceptance_gate_id": manifest_input.operator_acceptance_gate_id,
        "operator_acceptance_gate_date": manifest_input.operator_acceptance_gate_date,
        "generated_at_utc": manifest_input.generated_at_utc,
        "operator_acceptance_gate_state": operator_acceptance_gate_state,
        ...
        "final_operator_acceptance_gate_summary": _final_operator_acceptance_gate_summary(
            operator_acceptance_gate_state=operator_acceptance_gate_state,
    ...
    _validate_json_value("research_next_cycle_operator_acceptance_gate_payload", payload)
    return _normalize_json_value(payload)


def write_research_next_cycle_operator_acceptance_gate(
    manifest_input: ResearchNextCycleOperatorAcceptanceGateInput,
    *,
    out_dir: Path = DEFAULT_RESEARCH_NEXT_CYCLE_OPERATOR_ACCEPTANCE_GATE_DIR,
) -> tuple[Path, Path]:
    payload = build_research_next_cycle_operator_acceptance_gate_payload(manifest_input)
    out_dir.mkdir(parents=True, exist_ok=True)
    json_path = out_dir / RESEARCH_NEXT_CYCLE_OPERATOR_ACCEPTANCE_GATE_JSON
    markdown_path = out_dir / RESEARCH_NEXT_CYCLE_OPERATOR_ACCEPTANCE_GATE_MARKDOWN
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    markdown_path.write_text(render_research_next_cycle_operator_acceptance_gate_markdown(payload), encoding="utf-8")
    return json_path, markdown_path


def render_research_next_cycle_operator_acceptance_gate_markdown(payload: dict[str, Any]) -> str:
    _validate_json_value("research_next_cycle_operator_acceptance_gate_payload", payload)
    lines = [
        "# 26A Next Cycle Operator Acceptance Gate",
        "",
        f"Operator Acceptance Gate ID: {payload['operator_acceptance_gate_id']}",
        f"Operator Acceptance Gate Date: {payload['operator_acceptance_gate_date']}",
        f"Generated: {payload['generated_at_utc']}",
        f"Operator Acceptance Gate State: {payload['operator_acceptance_gate_state']}",
        "",
        "Status: RESEARCH_ONLY / MONITOR_ONLY / PAPER_ONLY / HUMAN_REVIEW_REQUIRED / BLOCKED_BY_SAFETY_GATE.",
        "Next-cycle operator acceptance gates are read-only and records-only.",
        ...
        "",
        "## Final operator acceptance gate Summary",
        "",
        payload["final_operator_acceptance_gate_summary"],
        "",
        "## Summary",
        "",
    ...
    lines.extend(
        [
            "## Safety Boundary",
            "",
            "- RESEARCH_ONLY operator acceptance gate generation only.",
...
def _source_artifacts(manifest_input: ResearchNextCycleOperatorAcceptanceGateInput) -> list[dict[str, Any]]:
    artifact_paths = (
        ("dry_run_manifest", "25B Next Cycle Dry Run Manifest", manifest_input.dry_run_manifest_path),
        ...
        ("archive_index", "23B Research Cycle Archive Index", manifest_input.archive_index_path),
        ("operations_console", "23A Research Operations Console", manifest_input.operations_console_path),
...
def _planned_steps(source_payloads: dict[str, dict[str, Any]], queue_status: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if isinstance(planned, list):
        for index, item in enumerate(planned, start=1):
            ...
            steps.append(
                {
                    "step_id": f"26A-STEP-{phase}",
                    "phase": phase,
                    "title": title,
                    "label": item.get("label", HUMAN_REVIEW_REQUIRED),
                    "status": "dry_run_planned_not_started",
                    "summary": item.get("summary", "Planned next-cycle workflow recorded as a acceptance-gate step."),
                    "would_run": False,
                    "run_started": False,
                    "records_only": True,
    ...
    if not steps:
        ...
        if isinstance(next_phase, dict):
            steps.append(
                {
                    "step_id": f"26A-STEP-{next_phase.get('phase', 'next_phase')}",
                    "phase": next_phase.get("phase"),
                    "title": next_phase.get("title", "Queued next phase"),
                    "label": HUMAN_REVIEW_REQUIRED,
                    "status": "dry_run_planned_not_started",
                    "summary": next_phase.get("summary", "Queued next phase recorded as a acceptance-gate step."),
                    "would_run": False,
                    "run_started": False,
                    "records_only": True,
                    "human_review_required": True,
    ...
    if not steps:
        steps.append(
            {
                "step_id": "26A-STEP-NEXT-CYCLE-SELECTION",
                "phase": None,
        ...
        hints.append(
            {
                ...
                "command_hint": f"# acceptance-gate hint only: review command for {phase}",
                "would_run": False,
                "executed": False,
                "records_only": True,
            }
...
def _expected_output_artifacts(planned_steps: list[dict[str, Any]]) -> list[dict[str, Any]]:
    outputs = [
        {
            "artifact_id": "26A_OPERATOR_ACCEPTANCE_GATE_json",
            "step_id": "26A-SELF",
            "label": RESEARCH_ONLY,
            "status": "expected_record",
            "summary": "26A JSON operator acceptance gate record.",
            "path": (
        ...
        {
            "artifact_id": "26A_OPERATOR_ACCEPTANCE_GATE_markdown",
            "step_id": "26A-SELF",
            "label": RESEARCH_ONLY,
            "status": "expected_record",
            "summary": "26A Markdown operator acceptance gate record.",
            "path": (
                DEFAULT_RESEARCH_NEXT_CYCLE_OPERATOR_ACCEPTANCE_GATE_DIR
                / RESEARCH_NEXT_CYCLE_OPERATOR_ACCEPTANCE_GATE_MARKDOWN
            ).as_posix(),
            "created_by_next_cycle": False,
        },
    ]
        ...
        outputs.append(
            {
                ...
                "summary": f"Expected future records for {step.get('title', phase)} are listed only; 26A does not create them.",
...
def _skipped_steps(
    ...
    reason_text = ", ".join(reasons) if reasons else "26A is acceptance-gate only"
...
def _required_operator_actions(
    *,
    ...
    actions = [
        _action(
            "26A-REVIEW-OPERATOR-ACCEPTANCE-GATE",
            "Review this 26A operator acceptance gate before any next-cycle start.",
        )
    ]
    if blocked_prerequisites:
        actions.append(_action("26A-RESOLVE-BLOCKED-PREREQUISITES", "Resolve blocked prerequisites while keeping safety gates active."))
    if missing_artifacts:
        actions.append(_action("26A-RESOLVE-MISSING-ARTIFACTS", "Regenerate or explicitly accept missing source artifacts."))
    if stale_artifacts or required_refreshed_artifacts:
        actions.append(_action("26A-REFRESH-STALE-ARTIFACTS", "Refresh stale artifacts or record human acceptance."))
    if required_operator_review_items:
        actions.append(_action("26A-COMPLETE-OPERATOR-REVIEW", "Complete operator review items before accepting the next cycle."))
    if safety_findings:
        actions.append(_action("26A-RESOLVE-SAFETY-FINDINGS", "Resolve safety findings while leaving live trading disabled."))
...
def _acceptance_criteria(
    *,
    ...
    return _dedupe_by_id(
        [
            _criterion("26A-ACCEPT-RECORDS-ONLY", "26A produces JSON and Markdown records only and does not run the next cycle.", RESEARCH_ONLY),
            _criterion("26A-ACCEPT-COMMAND-HINTS-INERT", "Command hints are recorded as inert text with would_run=false and executed=false.", HUMAN_REVIEW_REQUIRED),
            _criterion("26A-ACCEPT-SAFETY-LABELS", "Required safety labels and LIVE TRADING: DISABLED are present.", HUMAN_REVIEW_REQUIRED),
            _criterion("26A-ACCEPT-BLOCKED-PREREQS", f"Blocked prerequisite count is {len(blocked_prerequisites)} and remains blocked until human review.", BLOCKED_BY_SAFETY_GATE if blocked_prerequisites else HUMAN_REVIEW_REQUIRED),
            _criterion("26A-ACCEPT-REFRESHES", f"Required refreshed artifact count is {len(required_refreshed_artifacts)} and must be resolved or accepted.", HUMAN_REVIEW_REQUIRED),
            _criterion("26A-ACCEPT-OPERATOR-REVIEW", f"Required operator review item count is {len(required_operator_review_items)}.", HUMAN_REVIEW_REQUIRED),
            _criterion("26A-ACCEPT-SKIPPED-STEPS", f"Skipped step count is {len(skipped_steps)} and no planned step is executed.", PAPER_ONLY),
        ],
        "criterion_id",
    )


def _queue_next_phase(queue_status: dict[str, Any], dry_run_manifest_payload: dict[str, Any]) -> dict[str, Any]:
    next_phase = queue_status.get("next_phase")
    planned = dry_run_manifest_payload.get("planned_next_cycle_steps", [])
    if isinstance(planned, list):
        for item in planned:
    ...
    if not isinstance(next_phase, dict):
        return {
            ...
            "summary": "No queue next phase was available for 26A.",
            "run_started": False,
        }
    value = {
        "workflow_id": "queue_next_phase",
...
def _operator_acceptance_gate_state(
    *,
    missing_artifacts: list[dict[str, Any]],
    stale_artifacts: list[dict[str, Any]],
    safety_findings: list[dict[str, Any]],
    ...
    return ACCEPTANCE_READY_FOR_HUMAN_REVIEW
...
def _queue_status(path: Path) -> dict[str, Any]:
    ...
    value = {
        ...
        "label": HUMAN_REVIEW_REQUIRED,
        "status": "read_only",
        "summary": "Master plan queue read for 26A operator acceptance gate context only.",
        "path": path.as_posix(),
...
def _safety_scanner_status(path: Path) -> dict[str, Any]:
    ...
    if payload is None:
        return {
            ...
            "label": HUMAN_REVIEW_REQUIRED,
            "status": "not_run",
            "summary": "Safety scanner status was not supplied to 26A.",
            "path": path.as_posix(),
            "finding_count": 0,
            "passed": None,
            "findings": [],
    ...
    value = {
        ...
        "summary": payload.get("summary", "Safety scanner status supplied to 26A."),
        "path": path.as_posix(),
...
def _final_operator_acceptance_gate_summary(
    *,
    operator_acceptance_gate_state: str,
    planned_steps: list[dict[str, Any]],
    ...
    return (
        f"Next-cycle operator acceptance gate state is {operator_acceptance_gate_state}. "
        ...
        "Safety boundary confirmed: 26A is acceptance-gate only and records-only and does not execute commands, run the next cycle, mutate artifacts, delete artifacts, create broker actions, create trade instructions, enable live trading, grant execution permissions, or submit any order path. "
        "LIVE TRADING: DISABLED."
    )
...
def _validate_dry_run_path(path: Path) -> None:
    if path.name in SECRET_FILE_NAMES:
        raise ValueError("research next cycle operator acceptance gate cannot target secret files")
    lowered_parts = [part.lower() for part in path.parts]
    if any(marker in part for part in lowered_parts for marker in SECRET_PATH_MARKERS):
        raise ValueError("research next cycle operator acceptance gate cannot target credential or secret paths")
    if path.is_absolute():
        raise ValueError("research next cycle operator acceptance gate paths must be repo-relative")
    if ".." in path.parts:
        raise ValueError("research next cycle operator acceptance gate paths cannot traverse parent directories")
...
def _validate_json_value(field_name: str, value: Any) -> None:
    if isinstance(value, dict):
        ...
        if label is not None and label not in SAFE_DRY_RUN_LABELS:
            raise ValueError(f"unsafe research next cycle operator acceptance gate label: {label}")
        manifest_state = value.get("operator_acceptance_gate_state")
        if manifest_state is not None and manifest_state not in ACCEPTANCE_GATE_STATES:
            raise ValueError(f"invalid research next cycle operator acceptance gate state: {manifest_state}")
        for unsafe_field in UNSAFE_TRUE_FIELDS:
            if value.get(unsafe_field) is True:
                raise ValueError(f"research next cycle operator acceptance gate cannot set {unsafe_field}")
        for mutation_field in ("artifact_delete_performed", "artifact_mutation_performed", "commands_executed"):
            if value.get(mutation_field) is True:
                raise ValueError(f"research next cycle operator acceptance gate cannot set {mutation_field}")
    ...
    if isinstance(value, str):
        if value in DISALLOWED_DRY_RUN_LABELS:
            raise ValueError(f"disallowed research next cycle operator acceptance gate text: {value}")
        return
