...
DEFAULT_RESEARCH_NEXT_CYCLE_START_CHECKLIST_PACKET_DIR = Path(
    "reports/research_next_cycle_start_checklist_packet"
)
RESEARCH_NEXT_CYCLE_START_CHECKLIST_PACKET_JSON = "research_next_cycle_start_checklist_packet.json"
RESEARCH_NEXT_CYCLE_START_CHECKLIST_PACKET_MARKDOWN = "research_next_cycle_start_checklist_packet.md"

START_CHECKLIST_READY_FOR_HUMAN_REVIEW = "START_CHECKLIST_READY_FOR_HUMAN_REVIEW"
NEEDS_OPERATOR_REVIEW = "NEEDS_OPERATOR_REVIEW"

SAFE_START_CHECKLIST_LABELS = (
    RESEARCH_ONLY,
    MONITOR_ONLY,
    PAPER_ONLY,
    HUMAN_REVIEW_REQUIRED,
...
START_CHECKLIST_PACKET_STATES = (
    START_CHECKLIST_READY_FOR_HUMAN_REVIEW,
...
class ResearchNextCycleStartChecklistPacketInput:
    start_checklist_packet_id: str
    start_checklist_packet_date: str
    ...
    def validate(self) -> None:
        for field_name in ("start_checklist_packet_id", "start_checklist_packet_date", "generated_at_utc"):
            value = getattr(self, field_name)
            if not isinstance(value, str) or not value.strip():
                raise ValueError(f"research Next Cycle Start Checklist Packet requires {field_name}")
        _parse_iso_date("start_checklist_packet_date", self.start_checklist_packet_date)
...
def build_default_research_next_cycle_start_checklist_packet_input(
    *,
    start_checklist_packet_date: date | None = None,
    now: datetime | None = None,
) -> ResearchNextCycleStartChecklistPacketInput:
    generated = now or datetime.now(tz=UTC)
    day = start_checklist_packet_date or generated.date()
    return ResearchNextCycleStartChecklistPacketInput(
        start_checklist_packet_id=f"28B-RESEARCH-NEXT-CYCLE-START-CHECKLIST-PACKET-{day.isoformat()}",
        start_checklist_packet_date=day.isoformat(),
        generated_at_utc=generated.isoformat(),
    )


def build_research_next_cycle_start_checklist_packet_payload(
    packet_input: ResearchNextCycleStartChecklistPacketInput,
) -> dict[str, Any]:
    packet_input.validate()
    packet_day = datetime.strptime(packet_input.start_checklist_packet_date, "%Y-%m-%d").date()
    ...
    start_checklist_packet_state = _start_checklist_packet_state(
    ...
    payload = {
        "phase": "28B",
        "workflow": "Next Cycle Start Checklist Packet",
        "start_checklist_packet_id": packet_input.start_checklist_packet_id,
        "start_checklist_packet_date": packet_input.start_checklist_packet_date,
        "generated_at_utc": packet_input.generated_at_utc,
        "precondition_state": precondition_state,
        "start_checklist_packet_state": start_checklist_packet_state,
        "safety_boundary": _safety_boundary(),
        "required_labels": [*SAFE_START_CHECKLIST_LABELS, "LIVE TRADING: DISABLED"],
        "summary": {
        ...
        "final_next_cycle_start_checklist": _final_checklist(
            start_checklist_packet_state=start_checklist_packet_state,
            precondition_state=precondition_state,
    ...
    _validate_start_checklist_payload(payload)
    return _normalize_json_value(payload)


def write_research_next_cycle_start_checklist_packet(
    packet_input: ResearchNextCycleStartChecklistPacketInput,
    *,
    out_dir: Path = DEFAULT_RESEARCH_NEXT_CYCLE_START_CHECKLIST_PACKET_DIR,
) -> tuple[Path, Path]:
    payload = build_research_next_cycle_start_checklist_packet_payload(packet_input)
    out_dir.mkdir(parents=True, exist_ok=True)
    json_path = out_dir / RESEARCH_NEXT_CYCLE_START_CHECKLIST_PACKET_JSON
    markdown_path = out_dir / RESEARCH_NEXT_CYCLE_START_CHECKLIST_PACKET_MARKDOWN
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    markdown_path.write_text(render_research_next_cycle_start_checklist_packet_markdown(payload), encoding="utf-8")
    return json_path, markdown_path


def render_research_next_cycle_start_checklist_packet_markdown(payload: dict[str, Any]) -> str:
    _validate_start_checklist_payload(payload)
    lines = [
        "# 28B Next Cycle Start Checklist Packet",
        "",
        f"Start Checklist Packet ID: {payload['start_checklist_packet_id']}",
        f"Start Checklist Packet Date: {payload['start_checklist_packet_date']}",
        f"Generated: {payload['generated_at_utc']}",
        f"Precondition State: {payload['precondition_state']}",
        f"Start Checklist Packet State: {payload['start_checklist_packet_state']}",
        "",
        "Status: RESEARCH_ONLY / MONITOR_ONLY / PAPER_ONLY / HUMAN_REVIEW_REQUIRED / BLOCKED_BY_SAFETY_GATE.",
        "Next-cycle start checklist packets are read-only and records-only.",
        ...
        "",
        "## Final Next-Cycle Start Checklist",
        "",
        payload["final_next_cycle_start_checklist"],
        "",
        "## Summary",
        "",
    ...
    lines.extend(_section("Precondition State", [{"workflow_id": "precondition_state", "label": HUMAN_REVIEW_REQUIRED, "status": payload["precondition_state"], "summary": "28A precondition state recorded."}]))
    ...
    lines.extend(
        [
            "## Safety Boundary",
            "",
            "- RESEARCH_ONLY checklist packet generation only.",
...
def _source_artifacts(packet_input: ResearchNextCycleStartChecklistPacketInput) -> list[dict[str, Any]]:
    artifact_paths = (
...
def _planned_records_only_steps(
    ...
    for index, (artifact_id, item) in enumerate(source_steps, start=1):
        ...
        steps.append(
            {
                "step_id": f"28B-RECORDS-ONLY-STEP-{phase}-{index}",
    ...
    if not steps:
        ...
        if isinstance(next_phase, dict):
            steps.append(
                {
                    "step_id": f"28B-RECORDS-ONLY-STEP-{next_phase.get('phase') or 'NEXT'}",
...
def _inert_command_hints(
    ...
    for artifact_id in ("start_preconditions_gate", "operator_handoff_packet", "launch_control_gate", "acceptance_packet", "dry_run_manifest"):
        ...
        for index, item in enumerate(values, start=1):
            ...
            hints.append(
                {
                    "hint_id": f"28B-HINT-{artifact_id}-{index}",
                    "source_artifact_id": artifact_id,
            ...
            hints.append(
                {
                    ...
                    "command_hint": "Review source artifact only; do not run a workflow from 28B.",
                    "would_run": False,
                    "executed": False,
                    "records_only": True,
                    "execution_permission_granted": False,
...
def _blocked_prerequisites(
    ...
    blocked = [
        _blocked(f"missing_{item['artifact_id']}", f"{item['name']} is required before checklist acceptance.")
...
def _unresolved_review_items(
    ...
    for artifact_id, payload in source_payloads.items():
        for key in ("operator_review_items", "required_operator_actions", "required_human_review_actions", "unresolved_items"):
            for item in _items_from_payload(payload, key):
                items.append(
                    {
                        "review_item_id": f"28B-{artifact_id}-{key}-{item.get('review_item_id') or item.get('action_id') or item.get('unresolved_item_id') or len(items) + 1}",
                        "source_artifact_id": artifact_id,
                        "label": item.get("label", HUMAN_REVIEW_REQUIRED),
    ...
    for item in blocked_prerequisites:
        items.append(
            {
                "review_item_id": f"28B-BLOCKED-{item['prerequisite_id']}",
    ...
    for item in stale_artifacts:
        items.append(
            {
                "review_item_id": f"28B-STALE-{item['artifact_id']}",
...
def _required_refreshed_artifacts(
    ...
    for artifact_id, payload in source_payloads.items():
        for item in _items_from_payload(payload, "required_refreshed_artifacts"):
            refreshes.append(
                {
                    "refresh_id": f"28B-SOURCE-REFRESH-{artifact_id}-{item.get('refresh_id', len(refreshes) + 1)}",
...
def _safety_findings(
    ...
    for artifact_id, payload in source_payloads.items():
        for item in _items_from_payload(payload, "safety_findings"):
            findings.append(
                {
                    "finding_id": f"28B-{artifact_id}-{item.get('finding_id') or item.get('rule_id') or len(findings) + 1}",
                    "source_artifact_id": artifact_id,
...
def _operator_checklist_items(
    *,
    ...
    return _dedupe_by_id(
        [
            _checklist("28B-CHECK-PRECONDITION-STATE", f"28A precondition state recorded as {precondition_state}.", HUMAN_REVIEW_REQUIRED),
            _checklist("28B-CHECK-SOURCE-ARTIFACTS", f"{len(source_artifact_status)} source artifacts are listed with status.", HUMAN_REVIEW_REQUIRED),
            _checklist("28B-CHECK-INERT-COMMAND-HINTS", f"{len(command_hints)} inert command hints are listed with executed=false.", HUMAN_REVIEW_REQUIRED),
            _checklist("28B-CHECK-PLANNED-RECORDS-ONLY-STEPS", f"{len(planned_steps)} planned records-only steps are listed and not started.", PAPER_ONLY),
            _checklist("28B-CHECK-BLOCKED-PREREQUISITES", f"{len(blocked_prerequisites)} blocked prerequisites remain blocked.", BLOCKED_BY_SAFETY_GATE if blocked_prerequisites else HUMAN_REVIEW_REQUIRED),
            _checklist("28B-CHECK-UNRESOLVED-REVIEW", f"{len(unresolved_review_items)} unresolved review items require human review.", HUMAN_REVIEW_REQUIRED),
            _checklist("28B-CHECK-REFRESHED-ARTIFACTS", f"{len(required_refreshed_artifacts)} refreshed artifacts are required or explicitly accepted.", HUMAN_REVIEW_REQUIRED),
            _checklist("28B-CHECK-SAFETY-FINDINGS", f"{len(safety_findings)} safety findings are recorded.", BLOCKED_BY_SAFETY_GATE if safety_findings else HUMAN_REVIEW_REQUIRED),
            _checklist("28B-CHECK-QUEUE-NEXT-PHASE", f"Queue next phase recorded as {queue_next_phase.get('phase')}.", HUMAN_REVIEW_REQUIRED),
            _checklist("28B-CHECK-LIVE-TRADING-DISABLED", "LIVE TRADING: DISABLED; no execution permission, broker route, broker call, or order path is created.", HUMAN_REVIEW_REQUIRED),
        ],
        "checklist_item_id",
    )
...
def _acceptance_criteria(
    *,
    ...
    return _dedupe_by_id(
        [
            _criterion("28B-ACCEPT-RECORDS-ONLY", "28B produces deterministic JSON and Markdown records only.", RESEARCH_ONLY),
            _criterion("28B-ACCEPT-NO-WORKFLOW-START", "The next cycle is not run and no workflow is started.", HUMAN_REVIEW_REQUIRED),
            _criterion("28B-ACCEPT-INERT-COMMAND-HINTS", f"{len(command_hints)} command hints are inert records with executed=false.", HUMAN_REVIEW_REQUIRED),
            _criterion("28B-ACCEPT-PLANNED-STEPS", f"{len(planned_steps)} planned records-only steps are recorded with would_run=false.", PAPER_ONLY),
            _criterion("28B-ACCEPT-BLOCKED-PREREQUISITES", f"Blocked prerequisite count is {len(blocked_prerequisites)} and remains blocked.", BLOCKED_BY_SAFETY_GATE if blocked_prerequisites else HUMAN_REVIEW_REQUIRED),
            _criterion("28B-ACCEPT-UNRESOLVED-REVIEW", f"Unresolved review item count is {len(unresolved_review_items)}.", HUMAN_REVIEW_REQUIRED),
            _criterion("28B-ACCEPT-REFRESHED-ARTIFACTS", f"Required refreshed artifact count is {len(required_refreshed_artifacts)}.", HUMAN_REVIEW_REQUIRED),
            _criterion("28B-ACCEPT-SAFETY-FINDINGS", f"Safety finding count is {len(safety_findings)}.", BLOCKED_BY_SAFETY_GATE if safety_findings else HUMAN_REVIEW_REQUIRED),
            _criterion("28B-ACCEPT-OPERATOR-CHECKLIST", f"Operator checklist item count is {len(operator_checklist_items)}.", HUMAN_REVIEW_REQUIRED),
            _criterion("28B-ACCEPT-LIVE-TRADING-DISABLED", "LIVE TRADING: DISABLED is present and no broker/order path is created.", HUMAN_REVIEW_REQUIRED),
        ],
        "criterion_id",
    )
...
def _precondition_state(source_payloads: dict[str, dict[str, Any]]) -> str:
    payload = source_payloads.get("start_preconditions_gate", {})
    value = payload.get("start_preconditions_gate_state")
    return value if isinstance(value, str) and value.strip() else "START_PRECONDITIONS_STATE_NOT_SUPPLIED"


def _start_checklist_packet_state(
    *,
    missing_artifacts: list[dict[str, Any]],
    stale_artifacts: list[dict[str, Any]],
    safety_findings: list[dict[str, Any]],
    ...
    return START_CHECKLIST_READY_FOR_HUMAN_REVIEW
...
def _queue_status(path: Path) -> dict[str, Any]:
    ...
    value = {
        ...
        "label": HUMAN_REVIEW_REQUIRED,
        "status": "read_only",
        "summary": "Master plan queue read for 28B Start Checklist Packet context only.",
        "path": path.as_posix(),
...
def _queue_next_phase(queue_status: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(next_phase, dict):
        return {
            ...
            "summary": "No queue next phase was available to 28B.",
            "run_started": False,
        }
    value = {
        "workflow_id": "queue_next_phase",
        ...
        "summary": next_phase.get("summary") or "Queue next phase recorded for start checklist review only.",
        "run_started": False,
    }
...
def _safety_scanner_status(path: Path) -> dict[str, Any]:
    ...
    if payload is None:
        return {
            ...
            "label": HUMAN_REVIEW_REQUIRED,
            "status": "not_run",
            "summary": "Safety scanner status was not supplied to 28B.",
            "path": path.as_posix(),
            "finding_count": 0,
            "passed": None,
            "findings": [],
    ...
    value = {
        ...
        "summary": payload.get("summary", "Safety scanner status supplied to 28B."),
        "path": path.as_posix(),
...
def _final_checklist(
    *,
    start_checklist_packet_state: str,
    precondition_state: str,
    ...
    return (
        f"Next-cycle Start Checklist Packet State is {start_checklist_packet_state}. "
        ...
        "Safety boundary confirmed: 28B is read-only and records-only and does not execute commands, run the next cycle, mutate artifacts, delete artifacts, create broker actions, create trade instructions, enable live trading, grant execution permissions, route broker orders, call broker endpoints, or submit any order path. "
        "LIVE TRADING: DISABLED."
    )
...
def _criterion(criterion_id: str, summary: str, label: str) -> dict[str, Any]:
    return {
        ...
        "label": label,
        "status": "start_checklist_acceptance_required",
        "summary": summary,
    }
...
def _checklist(checklist_item_id: str, summary: str, label: str) -> dict[str, Any]:
    return {
        ...
        "label": label,
        "status": "checklist_review_required",
        "summary": summary,
        "records_only": True,
        "execution_permission_granted": False,
        "run_started": False,
...
def _validate_start_checklist_payload(value: Any) -> None:
    _validate_json_value("research_next_cycle_start_checklist_packet_payload", value)
    _validate_start_checklist_state(value)


def _validate_start_checklist_state(value: Any) -> None:
    if isinstance(value, dict):
        state = value.get("start_checklist_packet_state")
        if state is not None and state not in START_CHECKLIST_PACKET_STATES:
            raise ValueError(f"invalid research Next Cycle Start Checklist Packet state: {state}")
        for item in value.values():
            _validate_start_checklist_state(item)
