...
DEFAULT_RESEARCH_NEXT_CYCLE_ACCEPTANCE_PACKET_DIR = Path("reports/research_next_cycle_acceptance_packet")
RESEARCH_NEXT_CYCLE_ACCEPTANCE_PACKET_JSON = "research_next_cycle_acceptance_packet.json"
RESEARCH_NEXT_CYCLE_ACCEPTANCE_PACKET_MARKDOWN = "research_next_cycle_acceptance_packet.md"

ACCEPTANCE_PACKET_READY_FOR_HUMAN_REVIEW = "ACCEPTANCE_PACKET_READY_FOR_HUMAN_REVIEW"
ACCEPTANCE_PACKET_NEEDS_OPERATOR_REVIEW = "ACCEPTANCE_PACKET_NEEDS_OPERATOR_REVIEW"
...
ACCEPTANCE_PACKET_STATES = (
    ACCEPTANCE_PACKET_READY_FOR_HUMAN_REVIEW,
    ACCEPTANCE_PACKET_NEEDS_OPERATOR_REVIEW,
    BLOCKED_BY_SAFETY_GATE,
)
DISALLOWED_PACKET_LABELS = tuple(
    verb + suffix
    for verb, suffix in (
        ("BUY", "_NOW"),
        ("SELL", "_NOW"),
...
class ResearchNextCycleAcceptancePacketInput:
    acceptance_packet_id: str
    acceptance_packet_date: str
    ...
    def validate(self) -> None:
        for field_name in ("acceptance_packet_id", "acceptance_packet_date", "generated_at_utc"):
            value = getattr(self, field_name)
            if not isinstance(value, str) or not value.strip():
                raise ValueError(f"research next cycle acceptance packet requires {field_name}")
        _parse_iso_date("acceptance_packet_date", self.acceptance_packet_date)
        ...
        for path in (
            ...
            _validate_packet_path(path)


def build_default_research_next_cycle_acceptance_packet_input(
    *,
    acceptance_packet_date: date | None = None,
    now: datetime | None = None,
) -> ResearchNextCycleAcceptancePacketInput:
    generated = now or datetime.now(tz=UTC)
    day = acceptance_packet_date or generated.date()
    return ResearchNextCycleAcceptancePacketInput(
        acceptance_packet_id=f"26B-RESEARCH-NEXT-CYCLE-ACCEPTANCE-PACKET-{day.isoformat()}",
        acceptance_packet_date=day.isoformat(),
        generated_at_utc=generated.isoformat(),
    )


def build_research_next_cycle_acceptance_packet_payload(
    packet_input: ResearchNextCycleAcceptancePacketInput,
) -> dict[str, Any]:
    packet_input.validate()
    packet_day = datetime.strptime(packet_input.acceptance_packet_date, "%Y-%m-%d").date()
    ...
    payload = {
        "phase": "26B",
        "workflow": "Next Cycle Acceptance Packet",
        "acceptance_packet_id": packet_input.acceptance_packet_id,
        "acceptance_packet_date": packet_input.acceptance_packet_date,
        "generated_at_utc": packet_input.generated_at_utc,
        "acceptance_state": acceptance_state,
        "safety_boundary": _safety_boundary(),
        "required_labels": [*SAFE_PACKET_LABELS, "LIVE TRADING: DISABLED"],
        "summary": {
        ...
        "queue_status": queue_status,
        "safety_scanner_status": safety_scanner_status,
        "final_next_cycle_acceptance_summary": _final_summary(
            acceptance_state=acceptance_state,
    ...
    _validate_json_value("research_next_cycle_acceptance_packet_payload", payload)
    return _normalize_json_value(payload)


def write_research_next_cycle_acceptance_packet(
    packet_input: ResearchNextCycleAcceptancePacketInput,
    *,
    out_dir: Path = DEFAULT_RESEARCH_NEXT_CYCLE_ACCEPTANCE_PACKET_DIR,
) -> tuple[Path, Path]:
    payload = build_research_next_cycle_acceptance_packet_payload(packet_input)
    out_dir.mkdir(parents=True, exist_ok=True)
    json_path = out_dir / RESEARCH_NEXT_CYCLE_ACCEPTANCE_PACKET_JSON
    markdown_path = out_dir / RESEARCH_NEXT_CYCLE_ACCEPTANCE_PACKET_MARKDOWN
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    markdown_path.write_text(render_research_next_cycle_acceptance_packet_markdown(payload), encoding="utf-8")
    return json_path, markdown_path


def render_research_next_cycle_acceptance_packet_markdown(payload: dict[str, Any]) -> str:
    _validate_json_value("research_next_cycle_acceptance_packet_payload", payload)
    lines = [
        "# 26B Next Cycle Acceptance Packet",
        "",
        f"Acceptance Packet ID: {payload['acceptance_packet_id']}",
        f"Acceptance Packet Date: {payload['acceptance_packet_date']}",
        ...
        "Next-cycle acceptance packets are read-only and records-only.",
        ...
        "",
        "## Final Next-Cycle Acceptance Summary",
        "",
        payload["final_next_cycle_acceptance_summary"],
        "",
        "## Summary",
        "",
    ...
    lines.extend(
        [
            "## Safety Boundary",
            "",
            "- RESEARCH_ONLY acceptance packet generation only.",
            ...
            "- Records-only packet; no command execution, next-cycle run, artifact mutation, artifact deletion, broker action, trade instruction, execution permission, live-trading enablement, or order path submission is created.",
...
def _source_artifacts(packet_input: ResearchNextCycleAcceptancePacketInput) -> list[dict[str, Any]]:
    artifact_paths = (
        ...
        ("archive_index", "23B Research Cycle Archive Index", packet_input.archive_index_path),
        ("operations_console", "23A Research Operations Console", packet_input.operations_console_path),
...
def _planned_dry_run_steps(
    ...
    if isinstance(source_steps, list):
        for index, item in enumerate(source_steps, start=1):
            ...
            steps.append(
                {
                    "step_id": f"26B-DRY-RUN-STEP-{phase}",
    ...
    if not steps:
        ...
        if isinstance(next_phase, dict):
            steps.append(
                {
                    "step_id": f"26B-DRY-RUN-STEP-{next_phase.get('phase') or 'NEXT'}",
                    "source_step_id": None,
...
def _command_hints(
    ...
    if not hints:
        for step in planned_steps:
            hints.append(
                {
                    "hint_id": f"26B-HINT-{step['step_id']}",
                    "source_hint_id": None,
                    "step_id": step["step_id"],
                    "label": HUMAN_REVIEW_REQUIRED,
                    "status": "inert_record_only",
                    "summary": f"Inert operator hint for {step.get('phase') or step['step_id']}; no command is executed.",
                    "command_hint": f"review packet for {step.get('phase') or step['step_id']}",
                    "would_run": False,
                    "executed": False,
                    "records_only": True,
                }
...
def _skipped_steps(planned_steps: list[dict[str, Any]], missing_artifacts: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    return _dedupe_by_id(
        [
            {
                "skipped_step_id": f"26B-SKIPPED-{step['step_id']}",
                "step_id": step["step_id"],
                "label": label,
                "status": "skipped_by_records_only_packet",
                "summary": "Step is recorded for acceptance review only and is not run by 26B.",
                "would_run": False,
                "run_started": False,
                "executed": False,
            }
...
def _blocked_prerequisites(
    ...
    blocked = [
        _blocked(f"missing_{item['artifact_id']}", f"{item['name']} is required before next-cycle acceptance.")
    ...
    if safety_scanner_status.get("passed") is False:
        blocked.append(_blocked("safety_scanner_not_passed", "Safety scanner must pass before acceptance can proceed."))
        ...
        for item in _items_from_payload(payload, "required_refreshed_artifacts"):
            refreshes.append(
                _refresh(
                    ...
                    item.get("summary", "Source artifact requires refresh before next-cycle acceptance."),
...
def _required_human_review_actions(
    *,
    ...
    actions = [
        _action("26B-HUMAN-REVIEW-SAFETY-BOUNDARY", "Confirm 26B remains records-only and LIVE TRADING: DISABLED."),
        _action("26B-HUMAN-REVIEW-ACCEPTANCE-STATE", "Review acceptance state before any separate future workflow is considered."),
    ]
    if blocked_prerequisites:
        actions.append(_action("26B-HUMAN-REVIEW-BLOCKED-PREREQUISITES", f"Review {len(blocked_prerequisites)} blocked prerequisites."))
    if required_refreshed_artifacts:
        actions.append(_action("26B-HUMAN-REVIEW-REFRESHED-ARTIFACTS", f"Refresh or explicitly accept {len(required_refreshed_artifacts)} artifacts."))
    if operator_review_items:
        actions.append(_action("26B-HUMAN-REVIEW-OPERATOR-ITEMS", f"Review {len(operator_review_items)} operator review items."))
    if safety_findings:
        actions.append(_action("26B-HUMAN-REVIEW-SAFETY-FINDINGS", f"Resolve {len(safety_findings)} safety findings."))
    if unresolved_items:
        actions.append(_action("26B-HUMAN-REVIEW-UNRESOLVED-ITEMS", f"Resolve or accept {len(unresolved_items)} unresolved items."))
...
def _acceptance_criteria(
    *,
    ...
    return _dedupe_by_id(
        [
            _criterion("26B-ACCEPT-RECORDS-ONLY", "26B produces deterministic JSON and Markdown records only.", RESEARCH_ONLY),
            _criterion("26B-ACCEPT-NO-WORKFLOW-START", "The next cycle is not run and no workflow is started.", HUMAN_REVIEW_REQUIRED),
            _criterion("26B-ACCEPT-INERT-COMMAND-HINTS", f"{len(command_hints)} command hints are inert records with executed=false.", HUMAN_REVIEW_REQUIRED),
            _criterion("26B-ACCEPT-PLANNED-DRY-RUN-STEPS", f"{len(planned_steps)} planned dry-run steps are recorded with would_run=false.", PAPER_ONLY),
            _criterion("26B-ACCEPT-SKIPPED-STEPS", f"{len(skipped_steps)} steps are explicitly skipped by the records-only packet.", PAPER_ONLY),
            _criterion("26B-ACCEPT-BLOCKED-PREREQUISITES", f"Blocked prerequisite count is {len(blocked_prerequisites)} and remains blocked.", BLOCKED_BY_SAFETY_GATE if blocked_prerequisites else HUMAN_REVIEW_REQUIRED),
            _criterion("26B-ACCEPT-REFRESHED-ARTIFACTS", f"Required refreshed artifact count is {len(required_refreshed_artifacts)}.", HUMAN_REVIEW_REQUIRED),
            _criterion("26B-ACCEPT-OPERATOR-REVIEW", f"Operator review item count is {len(operator_review_items)}.", HUMAN_REVIEW_REQUIRED),
            _criterion("26B-ACCEPT-SAFETY-FINDINGS", f"Safety finding count is {len(safety_findings)}.", BLOCKED_BY_SAFETY_GATE if safety_findings else HUMAN_REVIEW_REQUIRED),
            _criterion("26B-ACCEPT-UNRESOLVED-ITEMS", f"Unresolved item count is {len(unresolved_items)}.", HUMAN_REVIEW_REQUIRED),
            _criterion("26B-ACCEPT-LIVE-TRADING-DISABLED", "LIVE TRADING: DISABLED is present and no broker/order path is created.", HUMAN_REVIEW_REQUIRED),
        ],
        "criterion_id",
    )
...
def _acceptance_state(
    *,
    ...
    if stale_artifacts or operator_review_items or unresolved_items:
        return ACCEPTANCE_PACKET_NEEDS_OPERATOR_REVIEW
    return ACCEPTANCE_PACKET_READY_FOR_HUMAN_REVIEW
...
def _queue_status(path: Path) -> dict[str, Any]:
    ...
    value = {
        ...
        "label": HUMAN_REVIEW_REQUIRED,
        "status": "read_only",
        "summary": "Master plan queue read for 26B acceptance packet context only.",
        "path": path.as_posix(),
...
def _safety_scanner_status(path: Path) -> dict[str, Any]:
    ...
    if payload is None:
        return {
            ...
            "label": HUMAN_REVIEW_REQUIRED,
            "status": "not_run",
            "summary": "Safety scanner status was not supplied to 26B.",
            "path": path.as_posix(),
            "finding_count": 0,
            "passed": None,
            "findings": [],
    ...
    value = {
        ...
        "summary": payload.get("summary", "Safety scanner status supplied to 26B."),
        "path": path.as_posix(),
...
def _final_summary(
    *,
    ...
    return (
        f"Next-cycle acceptance state is {acceptance_state}. "
        ...
        "Safety boundary confirmed: 26B is read-only and records-only and does not execute commands, run the next cycle, mutate artifacts, delete artifacts, create broker actions, create trade instructions, enable live trading, grant execution permissions, or submit any order path. "
        "LIVE TRADING: DISABLED."
    )
...
def _validate_packet_path(path: Path) -> None:
    if path.name in SECRET_FILE_NAMES:
        raise ValueError("research next cycle acceptance packet cannot target secret files")
    lowered_parts = [part.lower() for part in path.parts]
    if any(marker in part for part in lowered_parts for marker in SECRET_PATH_MARKERS):
        raise ValueError("research next cycle acceptance packet cannot target credential or secret paths")
    if path.is_absolute():
        raise ValueError("research next cycle acceptance packet paths must be repo-relative")
    if ".." in path.parts:
        raise ValueError("research next cycle acceptance packet paths cannot traverse parent directories")
...
def _validate_json_value(field_name: str, value: Any) -> None:
    if isinstance(value, dict):
        ...
        if label is not None and label not in SAFE_PACKET_LABELS:
            raise ValueError(f"unsafe research next cycle acceptance packet label: {label}")
        state = value.get("acceptance_state")
        if state is not None and state not in ACCEPTANCE_PACKET_STATES:
            raise ValueError(f"invalid research next cycle acceptance packet state: {state}")
        for unsafe_field in UNSAFE_TRUE_FIELDS:
            if value.get(unsafe_field) is True:
                raise ValueError(f"research next cycle acceptance packet cannot set {unsafe_field}")
        for mutation_field in ("artifact_delete_performed", "artifact_mutation_performed", "commands_executed"):
            if value.get(mutation_field) is True:
                raise ValueError(f"research next cycle acceptance packet cannot set {mutation_field}")
    ...
    if isinstance(value, str):
        if value in DISALLOWED_PACKET_LABELS:
            raise ValueError(f"disallowed research next cycle acceptance packet text: {value}")
        return
