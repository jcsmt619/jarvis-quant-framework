...
DEFAULT_RESEARCH_NEXT_CYCLE_RECORDS_EVIDENCE_PACKET_32_DIR = Path(
    "reports/research_next_cycle_records_evidence_packet_32"
)
RESEARCH_NEXT_CYCLE_RECORDS_EVIDENCE_PACKET_32_JSON = "research_next_cycle_records_evidence_packet_32.json"
RESEARCH_NEXT_CYCLE_RECORDS_EVIDENCE_PACKET_32_MARKDOWN = "research_next_cycle_records_evidence_packet_32.md"

RECORDS_EVIDENCE_PACKET_32_READY_FOR_HUMAN_REVIEW = "RECORDS_EVIDENCE_PACKET_32_READY_FOR_HUMAN_REVIEW"
RECORDS_EVIDENCE_PACKET_32_NEEDS_OPERATOR_REVIEW = "RECORDS_EVIDENCE_PACKET_32_NEEDS_OPERATOR_REVIEW"

SAFE_RECORDS_EVIDENCE_PACKET_32_LABELS = (
    RESEARCH_ONLY,
    MONITOR_ONLY,
    PAPER_ONLY,
    HUMAN_REVIEW_REQUIRED,
...
RECORDS_EVIDENCE_PACKET_32_STATES = (
    RECORDS_EVIDENCE_PACKET_32_READY_FOR_HUMAN_REVIEW,
    RECORDS_EVIDENCE_PACKET_32_NEEDS_OPERATOR_REVIEW,
    BLOCKED_BY_SAFETY_GATE,
)


@dataclass(frozen=True)
class ResearchNextCycleRecordsEvidencePacket32Input:
    records_evidence_packet_32_id: str
    records_evidence_packet_32_date: str
    ...
    def validate(self) -> None:
        for field_name in ("records_evidence_packet_32_id", "records_evidence_packet_32_date", "generated_at_utc"):
            value = getattr(self, field_name)
            if not isinstance(value, str) or not value.strip():
                raise ValueError(f"research Next Cycle Records Evidence Packet 32 requires {field_name}")
        _parse_iso_date("records_evidence_packet_32_date", self.records_evidence_packet_32_date)
...
def build_default_research_next_cycle_records_evidence_packet_32_input(
    *,
    records_evidence_packet_32_date: date | None = None,
    now: datetime | None = None,
) -> ResearchNextCycleRecordsEvidencePacket32Input:
    generated = now or datetime.now(tz=UTC)
    day = records_evidence_packet_32_date or generated.date()
    return ResearchNextCycleRecordsEvidencePacket32Input(
        records_evidence_packet_32_id=f"32B-RESEARCH-NEXT-CYCLE-RECORDS-EVIDENCE-PACKET-32-{day.isoformat()}",
        records_evidence_packet_32_date=day.isoformat(),
        generated_at_utc=generated.isoformat(),
    )


def build_research_next_cycle_records_evidence_packet_32_payload(
    gate_input: ResearchNextCycleRecordsEvidencePacket32Input,
) -> dict[str, Any]:
    gate_input.validate()
    gate_day = datetime.strptime(gate_input.records_evidence_packet_32_date, "%Y-%m-%d").date()
    ...
    records_evidence_packet_32_state = _records_evidence_packet_32_state(
    ...
    payload = {
        "phase": "32B",
        "workflow": "Next Cycle Records Evidence Packet 32",
        "records_evidence_packet_32_id": gate_input.records_evidence_packet_32_id,
        "records_evidence_packet_32_date": gate_input.records_evidence_packet_32_date,
        ...
        "required_labels": [*SAFE_RECORDS_EVIDENCE_PACKET_32_LABELS, "LIVE TRADING: DISABLED"],
        "summary": {
        ...
        "final_records_evidence_packet_32_summary": _final_summary(
            records_evidence_packet_32_state=records_evidence_packet_32_state,
            records_review_gate_32_state=records_review_gate_32_state,
            records_evidence_state=records_evidence_state,
            records_review_state=records_review_state,
    ...
    _validate_records_evidence_packet_32_payload(payload)
    return _normalize_json_value(payload)


def write_research_next_cycle_records_evidence_packet_32(
    gate_input: ResearchNextCycleRecordsEvidencePacket32Input,
    *,
    out_dir: Path = DEFAULT_RESEARCH_NEXT_CYCLE_RECORDS_EVIDENCE_PACKET_32_DIR,
) -> tuple[Path, Path]:
    payload = build_research_next_cycle_records_evidence_packet_32_payload(gate_input)
    out_dir.mkdir(parents=True, exist_ok=True)
    json_path = out_dir / RESEARCH_NEXT_CYCLE_RECORDS_EVIDENCE_PACKET_32_JSON
    markdown_path = out_dir / RESEARCH_NEXT_CYCLE_RECORDS_EVIDENCE_PACKET_32_MARKDOWN
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    markdown_path.write_text(render_research_next_cycle_records_evidence_packet_32_markdown(payload), encoding="utf-8")
    return json_path, markdown_path


def render_research_next_cycle_records_evidence_packet_32_markdown(payload: dict[str, Any]) -> str:
    _validate_records_evidence_packet_32_payload(payload)
    lines = [
        "# 32B Next Cycle Records Evidence Packet 32",
        "",
        f"Records Evidence Packet 32 ID: {payload['records_evidence_packet_32_id']}",
        f"Records Evidence Packet 32 Date: {payload['records_evidence_packet_32_date']}",
        ...
        "Records Evidence Packet 32 is read-only and records-only.",
        ...
        "",
        "## Final Records Evidence Packet 32 Summary",
        "",
        payload["final_records_evidence_packet_32_summary"],
        "",
        "## Summary",
        "",
    ...
    lines.extend(
        [
            "## Safety Boundary",
            "",
            "- RESEARCH_ONLY Records Evidence Packet 32 generation only.",
...
def _source_artifacts(gate_input: ResearchNextCycleRecordsEvidencePacket32Input) -> list[dict[str, Any]]:
    artifact_paths = (
        (
            "records_review_gate_32",
...
def _operator_checklist_items(
    *,
    ...
    return _dedupe_by_id(
        [
            _checklist("32B-CHECK-RECORDS-REVIEW-GATE-32-STATE", f"Human operator reviews 32A records review gate state: {records_review_gate_32_state}.", HUMAN_REVIEW_REQUIRED),
            _checklist("32B-CHECK-RECORDS-EVIDENCE-STATE", f"Human operator reviews 31B records evidence state: {records_evidence_state}.", HUMAN_REVIEW_REQUIRED),
            _checklist("32B-CHECK-RECORDS-REVIEW-STATE", f"Human operator reviews 31A records review state: {records_review_state}.", HUMAN_REVIEW_REQUIRED),
            _checklist("32B-CHECK-EVIDENCE-STATE", f"Human operator reviews 30B evidence state: {evidence_state}.", HUMAN_REVIEW_REQUIRED),
            _checklist("32B-CHECK-REVIEW-STATE", f"Human operator reviews 30A review state: {review_state}.", HUMAN_REVIEW_REQUIRED),
            _checklist("32B-CHECK-FROZEN-LAUNCH", f"Human operator reviews 29B frozen launch state: {frozen_launch_state}.", HUMAN_REVIEW_REQUIRED),
            _checklist("32B-CHECK-AUTHORIZATION", f"Human operator reviews 29A authorization state: {authorization_state}.", HUMAN_REVIEW_REQUIRED),
            _checklist("32B-CHECK-CHECKLIST", f"Human operator reviews 28B checklist state: {checklist_state}.", HUMAN_REVIEW_REQUIRED),
            _checklist("32B-CHECK-PRECONDITIONS", f"Human operator reviews 28A precondition state: {precondition_state}.", HUMAN_REVIEW_REQUIRED),
            _checklist("32B-CHECK-SOURCE-ARTIFACTS", f"Human operator reviews {len(source_artifact_status)} source artifact statuses.", HUMAN_REVIEW_REQUIRED),
            _checklist("32B-CHECK-INERT-HINTS", f"Human operator confirms {len(inert_command_hints)} inert command hints have executed=false.", HUMAN_REVIEW_REQUIRED),
            _checklist("32B-CHECK-BLOCKED-PREREQUISITES", f"Human operator reviews {len(blocked_prerequisites)} blocked prerequisites.", BLOCKED_BY_SAFETY_GATE if blocked_prerequisites else HUMAN_REVIEW_REQUIRED),
            _checklist("32B-CHECK-UNRESOLVED-REVIEW", f"Human operator resolves or accepts {len(unresolved_review_items)} unresolved review items.", HUMAN_REVIEW_REQUIRED),
            _checklist("32B-CHECK-REFRESHED-ARTIFACTS", f"Human operator refreshes or accepts {len(required_refreshed_artifacts)} required artifacts.", HUMAN_REVIEW_REQUIRED),
            _checklist("32B-CHECK-SAFETY-FINDINGS", f"Human operator reviews {len(safety_findings)} safety findings.", BLOCKED_BY_SAFETY_GATE if safety_findings else HUMAN_REVIEW_REQUIRED),
            _checklist("32B-CHECK-QUEUE-NEXT-PHASE", f"Human operator confirms queue next phase remains records-only and not started: {queue_next_phase.get('phase')}.", HUMAN_REVIEW_REQUIRED),
            _checklist("32B-CHECK-LIVE-TRADING-DISABLED", "Human operator confirms LIVE TRADING: DISABLED; no execution permission, broker route, broker call, or order path is created.", HUMAN_REVIEW_REQUIRED),
        ],
        "checklist_item_id",
    )
...
def _evidence_references(
    ...
    refs = [
        {
            "reference_id": f"32B-EVIDENCE-{item['artifact_id']}",
            "artifact_id": item["artifact_id"],
            "label": item.get("label", HUMAN_REVIEW_REQUIRED),
            "status": item["status"],
            "summary": item["summary"],
    ...
    refs.extend(
        [
            {
                "reference_id": "32B-EVIDENCE-master_plan_queue",
            ...
            {
                "reference_id": "32B-EVIDENCE-safety_scanner",
...
def _required_human_review_actions(
    *,
    ...
    return _dedupe_by_id(
        [
            _action("32B-ACTION-OPERATOR-CHECKLIST", f"Complete or explicitly defer {len(operator_checklist_items)} operator checklist items."),
            _action("32B-ACTION-EVIDENCE-REFERENCES", f"Review {len(evidence_references)} evidence references."),
            _action("32B-ACTION-BLOCKED-PREREQUISITES", f"Resolve or continue blocking {len(blocked_prerequisites)} blocked prerequisites.", BLOCKED_BY_SAFETY_GATE if blocked_prerequisites else HUMAN_REVIEW_REQUIRED),
            _action("32B-ACTION-UNRESOLVED-REVIEW", f"Resolve or accept {len(unresolved_review_items)} unresolved review items."),
            _action("32B-ACTION-REFRESHED-ARTIFACTS", f"Refresh or accept {len(required_refreshed_artifacts)} source artifact requirements."),
            _action("32B-ACTION-SAFETY-FINDINGS", f"Review {len(safety_findings)} safety findings.", BLOCKED_BY_SAFETY_GATE if safety_findings else HUMAN_REVIEW_REQUIRED),
            _action("32B-ACTION-QUEUE-NEXT-PHASE", f"Confirm queue next phase remains records-only and not started: {queue_next_phase.get('phase')}."),
            _action("32B-ACTION-NO-EXECUTION-PERMISSION", "Confirm records do not grant execution permission or start any workflow."),
            _action("32B-ACTION-LIVE-TRADING-DISABLED", "Confirm LIVE TRADING: DISABLED and no broker/order path is created."),
        ],
        "action_id",
    )


def _records_evidence_packet_32_state(
    *,
    ...
    if stale_artifacts or unresolved_review_items:
        return RECORDS_EVIDENCE_PACKET_32_NEEDS_OPERATOR_REVIEW
    return RECORDS_EVIDENCE_PACKET_32_READY_FOR_HUMAN_REVIEW
...
def _queue_status(path: Path) -> dict[str, Any]:
    ...
    value = {
        ...
        "status": "read_only",
        "summary": "Master plan queue read for 32B Records Evidence Packet 32 context only.",
        "path": path.as_posix(),
...
def _queue_next_phase(queue_status: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(next_phase, dict):
        return {
            ...
            "summary": "No queue next phase was available to 32B.",
            "run_started": False,
        }
    value = {
        "workflow_id": "queue_next_phase",
        "label": HUMAN_REVIEW_REQUIRED,
        "status": "records_evidence_planned_not_started",
        "phase": next_phase.get("phase"),
        "title": next_phase.get("title"),
        "summary": next_phase.get("summary") or "Queue next phase recorded for Records Evidence Packet 32 only.",
        "run_started": False,
    }
...
def _safety_scanner_status(path: Path) -> dict[str, Any]:
    ...
    if payload is None:
        return {
            ...
            "label": HUMAN_REVIEW_REQUIRED,
            "status": "not_run",
            "summary": "Safety scanner status was not supplied to 32B.",
            "path": path.as_posix(),
            "finding_count": 0,
            "passed": None,
            "findings": [],
    ...
    value = {
        ...
        "summary": payload.get("summary", "Safety scanner status supplied to 32B."),
        "path": path.as_posix(),
...
def _final_summary(
    *,
    ...
    return (
        f"Next-cycle Records Evidence Packet 32 State is {records_evidence_packet_32_state}. "
        ...
        "Safety boundary confirmed: 32B is read-only and records-only and does not execute commands, run the next cycle, mutate artifacts, delete artifacts, create broker actions, create trade instructions, enable live trading, grant execution permissions, route broker orders, call broker endpoints, or submit any order path. "
        "LIVE TRADING: DISABLED."
    )
...
def _checklist(checklist_item_id: str, summary: str, label: str) -> dict[str, Any]:
    return {
        ...
        "label": label,
        "status": "records_evidence_packet_32_required",
        "summary": summary,
        "records_only": True,
        "execution_permission_granted": False,
        "run_started": False,
...
def _validate_records_evidence_packet_32_payload(value: Any) -> None:
    _validate_json_value("research_next_cycle_records_evidence_packet_32_payload", value)
    _validate_records_evidence_packet_32_state(value)


def _validate_records_evidence_packet_32_state(value: Any) -> None:
    if isinstance(value, dict):
        state = value.get("records_evidence_packet_32_state")
        if state is not None and state not in RECORDS_EVIDENCE_PACKET_32_STATES:
            raise ValueError(f"invalid research Next Cycle Records Evidence Packet 32 state: {state}")
        for item in value.values():
            _validate_records_evidence_packet_32_state(item)
