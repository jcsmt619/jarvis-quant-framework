...
DEFAULT_RESEARCH_NEXT_CYCLE_START_PRECONDITIONS_GATE_DIR = Path("reports/research_next_cycle_start_preconditions_gate")
RESEARCH_NEXT_CYCLE_START_PRECONDITIONS_GATE_JSON = "research_next_cycle_start_preconditions_gate.json"
RESEARCH_NEXT_CYCLE_START_PRECONDITIONS_GATE_MARKDOWN = "research_next_cycle_start_preconditions_gate.md"

START_PRECONDITIONS_READY_FOR_HUMAN_REVIEW = "START_PRECONDITIONS_READY_FOR_HUMAN_REVIEW"
NEEDS_OPERATOR_REVIEW = "NEEDS_OPERATOR_REVIEW"

SAFE_START_PRECONDITIONS_LABELS = (
    RESEARCH_ONLY,
    MONITOR_ONLY,
    PAPER_ONLY,
    HUMAN_REVIEW_REQUIRED,
...
START_PRECONDITIONS_GATE_STATES = (
    START_PRECONDITIONS_READY_FOR_HUMAN_REVIEW,
    NEEDS_OPERATOR_REVIEW,
    BLOCKED_BY_SAFETY_GATE,
)
DISALLOWED_START_PRECONDITIONS_LABELS = tuple(
    verb + suffix
    for verb, suffix in (
        ("BUY", "_NOW"),
        ("SELL", "_NOW"),
...
class ResearchNextCycleStartPreconditionsGateInput:
    start_preconditions_gate_id: str
    start_preconditions_gate_date: str
    ...
    def validate(self) -> None:
        for field_name in ("start_preconditions_gate_id", "start_preconditions_gate_date", "generated_at_utc"):
            value = getattr(self, field_name)
            if not isinstance(value, str) or not value.strip():
                raise ValueError(f"research Next Cycle Start Preconditions Gate requires {field_name}")
        _parse_iso_date("start_preconditions_gate_date", self.start_preconditions_gate_date)
...
def build_default_research_next_cycle_start_preconditions_gate_input(
    *,
    start_preconditions_gate_date: date | None = None,
    now: datetime | None = None,
) -> ResearchNextCycleStartPreconditionsGateInput:
    generated = now or datetime.now(tz=UTC)
    day = start_preconditions_gate_date or generated.date()
    return ResearchNextCycleStartPreconditionsGateInput(
        start_preconditions_gate_id=f"28A-RESEARCH-NEXT-CYCLE-START-PRECONDITIONS-GATE-{day.isoformat()}",
        start_preconditions_gate_date=day.isoformat(),
        generated_at_utc=generated.isoformat(),
    )


def build_research_next_cycle_start_preconditions_gate_payload(
    packet_input: ResearchNextCycleStartPreconditionsGateInput,
) -> dict[str, Any]:
    packet_input.validate()
    packet_day = datetime.strptime(packet_input.start_preconditions_gate_date, "%Y-%m-%d").date()
    ...
    handoff_state = _source_handoff_state(source_payloads)
    acceptance_state = _source_acceptance_state(source_payloads)
    launch_control_state = _source_launch_control_state(source_payloads)
    dry_run_state = _source_dry_run_state(source_payloads)
    ...
    accepted_source_artifacts = _accepted_source_artifacts(source_artifacts)
    ...
    start_preconditions_criteria = _start_preconditions_criteria(
    ...
    start_preconditions_gate_state = _start_preconditions_gate_state(
    ...
    start_preconditions_checklist = _start_preconditions_checklist(
    ...
    payload = {
        "phase": "28A",
        "workflow": "Next Cycle Start Preconditions Gate",
        "start_preconditions_gate_id": packet_input.start_preconditions_gate_id,
        "start_preconditions_gate_date": packet_input.start_preconditions_gate_date,
        "generated_at_utc": packet_input.generated_at_utc,
        "handoff_state": handoff_state,
        "acceptance_state": acceptance_state,
        "launch_control_state": launch_control_state,
        "dry_run_state": dry_run_state,
        "start_preconditions_gate_state": start_preconditions_gate_state,
        "safety_boundary": _safety_boundary(),
        "required_labels": [*SAFE_START_PRECONDITIONS_LABELS, "LIVE TRADING: DISABLED"],
        "summary": {
            "source_artifact_count": len(source_artifacts),
            "accepted_source_artifact_count": len(accepted_source_artifacts),
            ...
            "start_preconditions_criteria_count": len(start_preconditions_criteria),
            ...
            "start_preconditions_checklist_count": len(start_preconditions_checklist),
            "handoff_state": handoff_state,
            "acceptance_state": acceptance_state,
            "launch_control_state": launch_control_state,
            "dry_run_state": dry_run_state,
            ...
            "label_counts": _count_by(
                [
                    ...
                    *operator_review_items,
                    *safety_findings,
                    *start_preconditions_criteria,
                    *unresolved_items,
                    *required_human_review_actions,
                    *start_preconditions_checklist,
                    queue_status,
        ...
        "operator_review_items": operator_review_items,
        "safety_findings": safety_findings,
        "start_preconditions_criteria": start_preconditions_criteria,
        "unresolved_items": unresolved_items,
        "required_human_review_actions": required_human_review_actions,
        "required_operator_actions": required_human_review_actions,
        "start_preconditions_checklist": start_preconditions_checklist,
        "queue_next_phase": queue_next_phase,
        "queue_status": queue_status,
        "safety_scanner_status": safety_scanner_status,
        "final_next_cycle_start_preconditions_summary": _final_summary(
            start_preconditions_gate_state=start_preconditions_gate_state,
            handoff_state=handoff_state,
            launch_control_state=launch_control_state,
            dry_run_state=dry_run_state,
    ...
    _validate_json_value("research_next_cycle_start_preconditions_gate_payload", payload)
    return _normalize_json_value(payload)


def write_research_next_cycle_start_preconditions_gate(
    packet_input: ResearchNextCycleStartPreconditionsGateInput,
    *,
    out_dir: Path = DEFAULT_RESEARCH_NEXT_CYCLE_START_PRECONDITIONS_GATE_DIR,
) -> tuple[Path, Path]:
    payload = build_research_next_cycle_start_preconditions_gate_payload(packet_input)
    out_dir.mkdir(parents=True, exist_ok=True)
    json_path = out_dir / RESEARCH_NEXT_CYCLE_START_PRECONDITIONS_GATE_JSON
    markdown_path = out_dir / RESEARCH_NEXT_CYCLE_START_PRECONDITIONS_GATE_MARKDOWN
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    markdown_path.write_text(render_research_next_cycle_start_preconditions_gate_markdown(payload), encoding="utf-8")
    return json_path, markdown_path


def render_research_next_cycle_start_preconditions_gate_markdown(payload: dict[str, Any]) -> str:
    _validate_json_value("research_next_cycle_start_preconditions_gate_payload", payload)
    lines = [
        "# 28A Next Cycle Start Preconditions Gate",
        "",
        f"Start Preconditions Gate ID: {payload['start_preconditions_gate_id']}",
        f"Start Preconditions Gate Date: {payload['start_preconditions_gate_date']}",
        ...
        f"Start Preconditions Gate State: {payload['start_preconditions_gate_state']}",
        "",
        "Status: RESEARCH_ONLY / MONITOR_ONLY / PAPER_ONLY / HUMAN_REVIEW_REQUIRED / BLOCKED_BY_SAFETY_GATE.",
        "Next-cycle start preconditions gates are read-only and records-only.",
        ...
        "",
        "## Final Next-Cycle Start Preconditions Summary",
        "",
        payload["final_next_cycle_start_preconditions_summary"],
        "",
        "## Summary",
        "",
        _summary_line("Source artifacts", payload["summary"]["source_artifact_count"]),
        _summary_line("Accepted source artifacts", payload["summary"]["accepted_source_artifact_count"]),
        ...
        _summary_line("Start preconditions criteria", payload["summary"]["start_preconditions_criteria_count"]),
        ...
        _summary_line("Start preconditions checklist items", payload["summary"]["start_preconditions_checklist_count"]),
        "",
    ]
    lines.extend(_section("Source Artifact Status", payload["source_artifacts"]))
    lines.extend(_section("Accepted Source Artifacts", payload["accepted_source_artifacts"]))
    ...
    lines.extend(_section("Start Preconditions Criteria", payload["start_preconditions_criteria"]))
    ...
    lines.extend(_section("Start Preconditions Checklist", payload["start_preconditions_checklist"]))
    ...
    lines.extend(
        [
            "## Safety Boundary",
            "",
            "- RESEARCH_ONLY Start Preconditions Gate generation only.",
            ...
            "- Records-only packet; no command execution, next-cycle run, artifact mutation, artifact deletion, broker action, trade instruction, execution permission, live-trading enablement, broker routing, broker endpoint call, or order path submission is created.",
...
def _source_artifacts(packet_input: ResearchNextCycleStartPreconditionsGateInput) -> list[dict[str, Any]]:
    artifact_paths = (
        ...
        ("operator_acceptance_gate", "26A Next Cycle Operator Acceptance Gate", packet_input.operator_acceptance_gate_path),
        ...
        ("rollover_gate", "24A Research Cycle Rollover Gate", packet_input.rollover_gate_path),
...
def _planned_dry_run_steps(
    ...
    if isinstance(source_steps, list):
        for index, item in enumerate(source_steps, start=1):
            ...
            steps.append(
                {
                    "step_id": f"28A-DRY-RUN-STEP-{phase}",
    ...
    if not steps:
        ...
        if isinstance(next_phase, dict):
            steps.append(
                {
                    "step_id": f"28A-DRY-RUN-STEP-{next_phase.get('phase') or 'NEXT'}",
                    "source_step_id": None,
...
def _command_hints(
    ...
    if not hints:
        for step in planned_steps:
            hints.append(
                {
                    "hint_id": f"28A-HINT-{step['step_id']}",
                    "source_hint_id": None,
...
def _skipped_steps(planned_steps: list[dict[str, Any]], missing_artifacts: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    return _dedupe_by_id(
        [
            {
                "skipped_step_id": f"28A-SKIPPED-{step['step_id']}",
                "step_id": step["step_id"],
                "label": label,
                "status": "skipped_by_records_only_gate",
                "summary": "Step is recorded for start preconditions review only and is not run by 28A.",
                "would_run": False,
                "run_started": False,
                "executed": False,
            }
...
def _required_human_review_actions(
    *,
    ...
    actions = [
        _action("28A-HUMAN-REVIEW-SAFETY-BOUNDARY", "Confirm 28A remains records-only and LIVE TRADING: DISABLED."),
        _action("28A-HUMAN-REVIEW-LAUNCH-CONTROL-STATE", "Review Start Preconditions Gate State before any separate future workflow is considered."),
    ]
    if blocked_prerequisites:
        actions.append(_action("28A-HUMAN-REVIEW-BLOCKED-PREREQUISITES", f"Review {len(blocked_prerequisites)} blocked prerequisites."))
    if required_refreshed_artifacts:
        actions.append(_action("28A-HUMAN-REVIEW-REFRESHED-ARTIFACTS", f"Refresh or explicitly accept {len(required_refreshed_artifacts)} artifacts."))
    if operator_review_items:
        actions.append(_action("28A-HUMAN-REVIEW-OPERATOR-ITEMS", f"Review {len(operator_review_items)} operator review items."))
    if safety_findings:
        actions.append(_action("28A-HUMAN-REVIEW-SAFETY-FINDINGS", f"Resolve {len(safety_findings)} safety findings."))
    if unresolved_items:
        actions.append(_action("28A-HUMAN-REVIEW-UNRESOLVED-ITEMS", f"Resolve or accept {len(unresolved_items)} unresolved items."))
    return _dedupe_by_id(actions, "action_id")


def _start_preconditions_criteria(
    *,
    planned_steps: list[dict[str, Any]],
    ...
    return _dedupe_by_id(
        [
            _criterion("28A-ACCEPT-RECORDS-ONLY", "28A produces deterministic JSON and Markdown records only.", RESEARCH_ONLY),
            _criterion("28A-ACCEPT-NO-WORKFLOW-START", "The next cycle is not run and no workflow is started.", HUMAN_REVIEW_REQUIRED),
            _criterion("28A-ACCEPT-INERT-COMMAND-HINTS", f"{len(command_hints)} command hints are inert records with executed=false.", HUMAN_REVIEW_REQUIRED),
            _criterion("28A-ACCEPT-PLANNED-DRY-RUN-STEPS", f"{len(planned_steps)} planned dry-run steps are recorded with would_run=false.", PAPER_ONLY),
            _criterion("28A-ACCEPT-SKIPPED-STEPS", f"{len(skipped_steps)} steps are explicitly skipped by the records-only gate.", PAPER_ONLY),
            _criterion("28A-ACCEPT-BLOCKED-PREREQUISITES", f"Blocked prerequisite count is {len(blocked_prerequisites)} and remains blocked.", BLOCKED_BY_SAFETY_GATE if blocked_prerequisites else HUMAN_REVIEW_REQUIRED),
            _criterion("28A-ACCEPT-REFRESHED-ARTIFACTS", f"Required refreshed artifact count is {len(required_refreshed_artifacts)}.", HUMAN_REVIEW_REQUIRED),
            _criterion("28A-ACCEPT-OPERATOR-REVIEW", f"Operator review item count is {len(operator_review_items)}.", HUMAN_REVIEW_REQUIRED),
            _criterion("28A-ACCEPT-SAFETY-FINDINGS", f"Safety finding count is {len(safety_findings)}.", BLOCKED_BY_SAFETY_GATE if safety_findings else HUMAN_REVIEW_REQUIRED),
            _criterion("28A-ACCEPT-UNRESOLVED-ITEMS", f"Unresolved item count is {len(unresolved_items)}.", HUMAN_REVIEW_REQUIRED),
            _criterion("28A-ACCEPT-LIVE-TRADING-DISABLED", "LIVE TRADING: DISABLED is present and no broker/order path is created.", HUMAN_REVIEW_REQUIRED),
        ],
        "criterion_id",
    )


def _start_preconditions_gate_state(
    *,
    missing_artifacts: list[dict[str, Any]],
    stale_artifacts: list[dict[str, Any]],
    safety_findings: list[dict[str, Any]],
    ...
    return START_PRECONDITIONS_READY_FOR_HUMAN_REVIEW
...
def _source_handoff_state(source_payloads: dict[str, dict[str, Any]]) -> str:
    handoff_payload = source_payloads.get("operator_handoff_packet", {})
    value = handoff_payload.get("operator_handoff_packet_state")
    if isinstance(value, str) and value.strip():
        return value
    return "HANDOFF_STATE_NOT_SUPPLIED"


def _source_launch_control_state(source_payloads: dict[str, dict[str, Any]]) -> str:
    launch_payload = source_payloads.get("launch_control_gate", {})
    value = launch_payload.get("launch_control_gate_state")
    if isinstance(value, str) and value.strip():
        return value
    return "LAUNCH_CONTROL_STATE_NOT_SUPPLIED"


def _source_dry_run_state(source_payloads: dict[str, dict[str, Any]]) -> str:
    dry_run_payload = source_payloads.get("dry_run_manifest", {})
    value = dry_run_payload.get("dry_run_manifest_state")
    if isinstance(value, str) and value.strip():
        return value
    return "DRY_RUN_STATE_NOT_SUPPLIED"
...
def _queue_next_phase(queue_status: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(next_phase, dict):
        return {
            ...
            "summary": "No queue next phase was available to 28A.",
            "run_started": False,
        }
    value = {
        "workflow_id": "queue_next_phase",
        ...
        "summary": next_phase.get("summary") or "Queue next phase recorded for start preconditions review only.",
        "run_started": False,
    }
...
def _queue_status(path: Path) -> dict[str, Any]:
    ...
    value = {
        ...
        "label": HUMAN_REVIEW_REQUIRED,
        "status": "read_only",
        "summary": "Master plan queue read for 28A Start Preconditions Gate context only.",
        "path": path.as_posix(),
...
def _safety_scanner_status(path: Path) -> dict[str, Any]:
    ...
    if payload is None:
        return {
            ...
            "label": HUMAN_REVIEW_REQUIRED,
            "status": "not_run",
            "summary": "Safety scanner status was not supplied to 28A.",
            "path": path.as_posix(),
            "finding_count": 0,
            "passed": None,
            "findings": [],
    ...
    value = {
        ...
        "summary": payload.get("summary", "Safety scanner status supplied to 28A."),
        "path": path.as_posix(),
...
def _accepted_source_artifacts(source_artifacts: list[dict[str, Any]]) -> list[dict[str, Any]]:
    accepted = []
    for item in source_artifacts:
        ...
        accepted.append(
            {
                ...
                "name": item["name"],
                "label": item.get("label", HUMAN_REVIEW_REQUIRED),
                "status": "accepted_for_preconditions_record",
                "summary": f"{item['name']} is accepted as read-only source context for 28A.",
                "path": item["path"],
    ...
    return _dedupe_by_id(accepted, "artifact_id")


def _start_preconditions_checklist(
    *,
    ...
    items = [
        _checklist("28A-CHECK-LAUNCH-CONTROL", f"Launch-control state recorded as {launch_control_state}.", HUMAN_REVIEW_REQUIRED),
        _checklist("28A-CHECK-SOURCE-ARTIFACTS", f"{len(accepted_source_artifacts)} source artifacts accepted for preconditions context.", HUMAN_REVIEW_REQUIRED),
        _checklist("28A-CHECK-RECORDS-ONLY-STEPS", f"{len(planned_steps)} planned records-only steps are listed and not started.", PAPER_ONLY),
        _checklist("28A-CHECK-INERT-COMMAND-HINTS", f"{len(command_hints)} inert command hints are listed with executed=false.", HUMAN_REVIEW_REQUIRED),
        _checklist("28A-CHECK-BLOCKED-PREREQUISITES", f"{len(blocked_prerequisites)} blocked prerequisites remain blocked.", BLOCKED_BY_SAFETY_GATE if blocked_prerequisites else HUMAN_REVIEW_REQUIRED),
        _checklist("28A-CHECK-UNRESOLVED-ITEMS", f"{len(unresolved_items)} unresolved review items require human review or acceptance.", HUMAN_REVIEW_REQUIRED),
        _checklist("28A-CHECK-REFRESHED-ARTIFACTS", f"{len(required_refreshed_artifacts)} refreshed artifacts are required or explicitly accepted.", HUMAN_REVIEW_REQUIRED),
        _checklist("28A-CHECK-SAFETY-FINDINGS", f"{len(safety_findings)} safety findings are recorded.", BLOCKED_BY_SAFETY_GATE if safety_findings else HUMAN_REVIEW_REQUIRED),
        _checklist("28A-CHECK-LIVE-TRADING-DISABLED", "LIVE TRADING: DISABLED; no execution permission, broker routing, broker call, or order path is created.", HUMAN_REVIEW_REQUIRED),
    ]
    return _dedupe_by_id(items, "checklist_item_id")


def _final_summary(
    *,
    start_preconditions_gate_state: str,
    handoff_state: str,
    launch_control_state: str,
    dry_run_state: str,
    ...
    operator_review_items: list[dict[str, Any]],
    safety_findings: list[dict[str, Any]],
    start_preconditions_criteria: list[dict[str, Any]],
    unresolved_items: list[dict[str, Any]],
    required_human_review_actions: list[dict[str, Any]],
    start_preconditions_checklist: list[dict[str, Any]],
) -> str:
    present = len([item for item in source_artifacts if item["status"] == "present"])
    return (
        f"Next-cycle Start Preconditions Gate State is {start_preconditions_gate_state}. "
        ...
        f"Accepted source artifacts: {len(accepted_source_artifacts)}. "
        ...
        f"Start preconditions criteria: {len(start_preconditions_criteria)}. "
        f"Unresolved items: {len(unresolved_items)}. "
        f"Required human-review actions: {len(required_human_review_actions)}. "
        f"Start preconditions checklist items: {len(start_preconditions_checklist)}. "
        "Safety boundary confirmed: 28A is read-only and records-only and does not execute commands, run the next cycle, mutate artifacts, delete artifacts, create broker actions, create trade instructions, enable live trading, grant execution permissions, route broker orders, call broker endpoints, or submit any order path. "
        "LIVE TRADING: DISABLED."
    )
...
def _criterion(criterion_id: str, summary: str, label: str) -> dict[str, Any]:
    return {
        ...
        "label": label,
        "status": "START_PRECONDITIONS_required",
        "summary": summary,
    }
...
def _checklist(checklist_item_id: str, summary: str, label: str) -> dict[str, Any]:
    return {
        ...
        "label": label,
        "status": "preconditions_review_required",
        "summary": summary,
        "records_only": True,
...
def _age_days(packet_day: date, generated_date: str | None) -> int | None:
    ...
    try:
        ...
        return None
    return (packet_day - generated_day).days
...
def _validate_gate_path(path: Path) -> None:
    if path.name in SECRET_FILE_NAMES:
        raise ValueError("research Next Cycle Start Preconditions Gate cannot target secret files")
    lowered_parts = [part.lower() for part in path.parts]
    if any(marker in part for part in lowered_parts for marker in SECRET_PATH_MARKERS):
        raise ValueError("research Next Cycle Start Preconditions Gate cannot target credential or secret paths")
    if path.is_absolute():
        raise ValueError("research Next Cycle Start Preconditions Gate paths must be repo-relative")
    if ".." in path.parts:
        raise ValueError("research Next Cycle Start Preconditions Gate paths cannot traverse parent directories")
...
def _validate_json_value(field_name: str, value: Any) -> None:
    if isinstance(value, dict):
        ...
        if label is not None and label not in SAFE_START_PRECONDITIONS_LABELS:
            raise ValueError(f"unsafe research Next Cycle Start Preconditions Gate label: {label}")
        state = value.get("start_preconditions_gate_state")
        if state is not None and state not in START_PRECONDITIONS_GATE_STATES:
            raise ValueError(f"invalid research Next Cycle Start Preconditions Gate state: {state}")
        for unsafe_field in UNSAFE_TRUE_FIELDS:
            if value.get(unsafe_field) is True:
                raise ValueError(f"research Next Cycle Start Preconditions Gate cannot set {unsafe_field}")
        for mutation_field in ("artifact_delete_performed", "artifact_mutation_performed", "commands_executed"):
            if value.get(mutation_field) is True:
                raise ValueError(f"research Next Cycle Start Preconditions Gate cannot set {mutation_field}")
    ...
    if isinstance(value, str):
        if value in DISALLOWED_START_PRECONDITIONS_LABELS:
            raise ValueError(f"disallowed research Next Cycle Start Preconditions Gate text: {value}")
        return
